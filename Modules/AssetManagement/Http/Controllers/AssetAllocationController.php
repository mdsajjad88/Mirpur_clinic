...
class AssetAllocationController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            $input = $request->only('ref_no', 'receiver', 'transaction_datetime', 'reason', 'allocated_upto');
            $input['transaction_type'] = 'allocate';
            $input['business_id'] = $business_id;
            $input['created_by'] = request()->session()->get('user.id');

            $asset_ids = $request->get('asset_id', []);
            ...
            foreach ($asset_ids as $index => $asset_id) {
                ...
                $transaction_data['asset_id'] = $asset_id;
                $transaction_data['quantity'] = $this->commonUtil->num_uf($quantities[$index] ?? 0);

                AssetTransaction::create($transaction_data);
            }
