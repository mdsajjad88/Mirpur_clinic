...
class AssetController extends Controller
{
            ...
            return Datatables::of($assets)
                ->addColumn('action', function ($row) {
                    ...
                    if (auth()->user()->can('asset.update')) {
                        $html .= '
                                <li>
                                    <a data-href="'.action([\Modules\AssetManagement\Http\Controllers\AssetController::class, 'show'], [$row->id]).'" class="cursor-pointer show_asset">
     ...
     */
     ...
     public function show($id)
     {
         ...
         $asset = Asset::leftJoin('categories', 'assets.category_id', '=', 'categories.id')
         ...
         $transactions = AssetTransaction::leftJoin('users as receiver', 'asset_transactions.receiver', '=', 'receiver.id')
             ->leftJoin('users as creator', 'asset_transactions.created_by', '=', 'creator.id')
             ->where('asset_transactions.asset_id', $id)
             ->where('asset_transactions.business_id', $business_id)
             ->select(
                 'asset_transactions.*',
                 DB::raw("CONCAT(COALESCE(receiver.first_name, ''), ' ', COALESCE(receiver.last_name, '')) as receiver_name"),
         ...
         return view('assetmanagement::asset.show', compact('asset', 'transactions'));
     }   
