<div class="row">
    <div class="col-sm-12">
        ...
        <table class="table table-condensed">
            ...
            @if ($register_details->total_return != 0)
                <tr class="danger">
                
                    <th>
                        @lang('cash_register.total_return')
                    </th>
                    <td>
                    
                    <b><span class="display_currency"
                        data-currency_symbol="true">{{ $register_details->total_return }}</span></b> <br>
                        <small>
                            @if ($register_details->total_cash_sell_return != 0)
                                Cash: <span class="display_currency"
                                    data-currency_symbol="true">{{ $register_details->total_cash_sell_return }}</span><br>
                            @endif
                            @if ($register_details->total_cheque_sell_return != 0)
                                Cheque: <span class="display_currency"
                                    data-currency_symbol="true">{{ $register_details->total_cheque_sell_return }}</span><br>
                            @endif
                            @if ($register_details->total_card_sell_return != 0)
                                Card: <span class="display_currency"
                                    data-currency_symbol="true">{{ $register_details->total_card_sell_return }}</span><br>
                            @endif
                            @if ($register_details->total_bank_transfer_sell_return != 0)
                                Bank Transfer: <span class="display_currency"
                                    data-currency_symbol="true">{{ $register_details->total_bank_transfer_sell_return }}</span><br>
                            @endif
                            @if (array_key_exists('custom_pay_1', $payment_types) && $register_details->total_custom_pay_1_sell_return != 0)
                                {{ $payment_types['custom_pay_1'] }}: <span class="display_currency"
                                    data-currency_symbol="true">{{ $register_details->total_custom_pay_1_sell_return }}</span>
                            @endif
                        </small>
                    </td>
                </tr>
...
@include('clinic::cash_register.register_product_details')