<table class="table table-bordered" id="contact_payments_table">
    ...
    <tbody>
        @forelse($payments as $payment)
            ...
            @endphp
            @include('clinic::patient.patients.partials.payment_row', compact('payment', 'count_child_payments', 'payment_types'))
            @if($count_child_payments > 0)
                @foreach($payment->child_payments as $child_payment)
                    @include('clinic::patient.patients.partials.payment_row', ['payment' => $child_payment, 'count_child_payments' => 0, 'payment_types' => $payment_types, 'parent_payment_ref_no' => $payment->payment_ref_no])
                @endforeach
            @endif
            @empty
                <tr>
