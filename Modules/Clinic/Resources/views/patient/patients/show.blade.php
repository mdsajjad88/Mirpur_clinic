...
@section('title', __('clinic::lang.customer_info'))
@section('active', 'patients')
@section('content')
        ...
        <div class="col-md-12">
            <div class="nav-tabs-custom">
                <ul class="nav nav-tabs nav-justified">
                    ...
                    <li class="
                        @if(!empty($view_type) &&  $view_type == 'appointment')
                            active
                        @else
                            ''
                        @endif">
                        <a href="#appointment_tab" data-toggle="tab" aria-expanded="true"><i class="fas fa-pen-square" aria-hidden="true"></i> @lang('clinic::lang.appointment')</a>
                        </li>
                    <li class="
                        @if(!empty($view_type) &&  $view_type == 'activities')
                            active
                        ...
                        <a href="#additional_discount_tab" data-toggle="tab" aria-expanded="true"><i class="fas fa-pen-square" aria-hidden="true"></i> @lang('clinic::lang.add_discount')</a>
                    </li>
                ...
                <div class="tab-content">
                    <div class="tab-pane
                                ...
                                @endif"
                            id="ledger_tab">
                        @include('clinic::patient.patients.partials.ledger_tab')
                    </div>
                    @if(in_array($contact->type, ['both', 'supplier']))
                        <div class="tab-pane
                            @if(!empty($view_type) &&  $view_type == 'purchase')
                    ...
                    @if(in_array($contact->type, ['both', 'customer']))
                        ...
                        id="sales_tab">
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    @include('clinic::patient.patients.partials.sells_table')
                                </div>
                            </div>
                        </div>
                        @if(in_array('subscription', $enabled_modules))
                    ...
                    <div class="tab-pane"
                        id="activities_tab">
                        @include('clinic::patient.patients.activity_log.activities')
                    </div>
                    <div class="tab-pane"
                        id="appointment_tab">
...
function get_contact_payments(url = null) {
    if (!url) {
        url = "{{action([\Modules\Clinic\Http\Controllers\PatientController::class, 'getContactPayments'], [$contact->id])}}";
    }
    $.ajax({
        url: url,
        dataType: 'html',
