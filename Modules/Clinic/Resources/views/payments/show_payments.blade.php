    ...
    <div class="modal-content">
        ...
        <div class="modal-body">
            @if(in_array($transaction->type, ['purchase', 'purchase_return']))
                <div class="row invoice-info">
                    <div class="col-sm-4 invoice-col">
                        @include('clinic::payments.transaction_supplier_details')
                    </div>
                    <div class="col-md-4 invoice-col">
                        @include('clinic::payments.payment_business_details')
                    </div>
            ...
            @can('send_notification')
                @if($transaction->type == 'purchase')
                    <div class="row no-print">
                        <div class="col-md-12 text-right">
                            ...
                            data-href="{{action([\Modules\Clinic\Http\Controllers\ClinicNotificationController::class, 'getTemplate'], ['transaction_id' => $transaction->id,'template_for' => 'payment_paid'])}}" data-container=".view_modal"><i class="fa fa-envelope"></i> @lang('lang_v1.payment_paid_notification')</button>
                        </div>
                    </div>
                    <br>
                @endif
                @if($transaction->type == 'sell' || $transaction->type == 'hms_booking')
                    <div class="row no-print">
                        <div class="col-md-12 text-right">
                            <button type="button" class="btn btn-info btn-modal btn-xs" 
                            data-href="{{action([\Modules\Clinic\Http\Controllers\ClinicNotificationController::class, 'getTemplate'], ['transaction_id' => $transaction->id,'template_for' => 'payment_received'])}}" data-container=".view_modal"><i class="fa fa-envelope"></i> @lang('lang_v1.payment_received_notification')</button>
                          
                            @if($transaction->payment_status != 'paid')
                                &nbsp;
                                <button type="button" class="btn btn-warning btn-modal btn-xs" data-href="{{action([\Modules\Clinic\Http\Controllers\ClinicNotificationController::class, 'getTemplate'], ['transaction_id' => $transaction->id,'template_for' => 'payment_reminder'])}}" data-container=".view_modal"><i class="fa fa-envelope"></i> @lang('lang_v1.send_payment_reminder')</button>
                            @endif
                        </div>
                    </div>
                    <br>
            ...
            @if($transaction->payment_status != 'paid')
                <div class="row mb-5">
                    <div class="col-md-12">
                        @if ($transaction->is_direct_sale == 0)
                            
                                <a data-is_direct_sale="{{ $transaction->is_direct_sale }}" href="{{ action([\Modules\Clinic\Http\Controllers\ClinicTransactionController::class, 'addPayment'], [$transaction->id]) }}" 
                        ...
                        @else
                                <a href="{{ action([\Modules\Clinic\Http\Controllers\ClinicTransactionController::class, 'addPayment'], [$transaction->id]) }}" 
                                   class="btn btn-primary btn-xs pull-right add_payment_modal no-print">
                                    <i class="fa fa-plus" aria-hidden="true"></i> @lang("purchase.add_payment")
                                </a>
                        ...
                        @forelse ($payments as $payment)
                            <tr>
                              ...
                              <td class="no-print" style="display: flex;">
                                    ...
                                    data-href="{{action([\Modules\Clinic\Http\Controllers\ClinicTransactionController::class, 'edit'], [$payment->id]) }}"><i class="glyphicon glyphicon-edit"></i></button>
                                    @endif
                               

                                
                                    &nbsp; <button type="button" class="btn btn-danger btn-xs delete_payment" 
                                    data-href="{{ action([\Modules\Clinic\Http\Controllers\ClinicTransactionController::class, 'destroy'], [$payment->id]) }}"
                                    ><i class="fa fa-trash" aria-hidden="true"></i></button>
                                
                              &nbsp;
                                <button type="button" class="btn btn-primary btn-xs view_payment" data-href="{{ action([\Modules\Clinic\Http\Controllers\ClinicTransactionController::class, 'viewPayment'], [$payment->id]) }}">
