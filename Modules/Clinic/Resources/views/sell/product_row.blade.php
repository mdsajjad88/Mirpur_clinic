...
<tr data-category="{{ $product->category_name }}" class="product_row" data-row_index="{{ $row_count }}"
    ...
    <td>
        ...
        @if (($edit_price || $edit_discount) && empty($is_direct_sell))
            <div title="@lang('lang_v1.pos_edit_product_price_help')">
                <span class="text-link text-info cursor-pointer" data-toggle="modal"
                    ...
                    {!! $product_name !!} @if (!empty($product->category_name))
		...
		@if ($product->product_type_for_clinic == 'consultation' || $product->product_type_for_clinic == 'therapy')
            <div style="display: flex; align-items: center; margin-bottom: 15px;">
                <label style="margin-right: 10px;" for="current_session">Session:<span style="color: red">*</span></label>
                <input 
                    type="number" 
                    name="products[{{ $row_count }}][current_session][{{ $product->variation_id }}]"
    ...
    <td>
        ...
        <div class="input-group input-number">
			...
			@else 
                ...
                @if ($product->enable_stock && empty($pos_settings['allow_overselling']) && empty($is_sales_order)) data-qty_available="{{ $product->qty_available }}" data-msg-max-value="{{ $max_qty_msg }}" 
        ...
        <span class="pos_qty_error error hide">Quantity {{ $product->formatted_qty_available }} {{ $product->unit }}
            available</span>
    ...
    @if (!empty($is_direct_sell))
        ...
        @endphp
        {{-- @if (auth()->user()->can('edit_product_price_from_sale_screen') || auth()->user()->can('view_product_price_from_sale_screen'))   --}}
        <td class="">
            <input type="text" name="products[{{ $row_count }}][unit_price]"
                class="form-control pos_unit_price input_number mousetrap"
                value="{{ @num_format($pos_unit_price) }}"@if (!auth()->user()->can('edit_product_price_from_sale_screen')) readonly @endif
    ...
    <td class="{{ $hide_tax }}">
        ...
        @if (!empty($discount))
            <small style="display: block; margin-top: 10px;">
                <strong>Camping Discount ({{ $discount->name }}):</strong>
                {{ number_format($discount_amount) }}{{ $discount_type == 'percentage' ? '%' : 'TK' }} <br>
                ({{ __('Validity') }}: {{ $discount->formated_ends_at }})
            </small>
        @endif
    </td>
    ...
    <td class="text-center" style="width: 12%">
        ...
        @if (in_array('modifiers', $enabled_modules))
            <br>
            <small class="total_modifier_row" style="font-size: 12px;">
                <strong>Options: </strong>
                <span class="display_currency total_modifier_price">৳ 0.00</span>
            </small>
        @endif
    </td>
