...
@section('content')
    ...
    <section class="content-header">
        <h1>@lang('clinic::test.test_list')
            <small>@lang('clinic::test.manage_test')</small>
        </h1>
        <!-- <ol class="breadcrumb">
            ...
            <div class="col-md-3">
                <div class="form-group">
                    <div class="checkbox">
                        <label>
                            <br>
                            {!! Form::checkbox('show_with_modifier', 1, false, ['class' => 'input-icheck', 'id' => 'show_with_modifier']) !!} {{ __('Show With Moodifier') }}
                        </label>
                    </div>
                </div>
            </div>
                    ...
                    <div class="nav-tabs-custom">
                        ...
                        <div class="tab-content">
                            <div class="tab-pane active" id="product_list_tab">
                                ...
                                @endif
                                @can('clinic.test.create')
                                    <a class="btn btn-primary pull-right"
                                        href="{{ action([\Modules\Clinic\Http\Controllers\TestController::class, 'create']) }}">
                                        <i class="fa fa-plus"></i> @lang('messages.add')</a>
                                    <br><br>
                                @endcan
                                @include('clinic::test.partials.product_list')
                            </div>
                            @if (auth()->user()->can('today.test.sell.details'))
                            ...
                            @if (auth()->user()->can('today.test.refund.details'))
                                <div class="tab-pane" id="refund_sell_tab">
                                    @include('clinic::test.bill-return-table')
                                </div>
                            @endif
...
@section('javascript')
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $(document).ready(function() {
                var url = "{{ route('get.today.therapy.sell.report') }}?sub_type=test";

                var ipd_sell_table = $('#product_sell_table').DataTable({
                    processing: true,
                    serverSide: true,
                    aaSorting: [
                        [4, 'desc']
                    ...
                    fnDrawCallback: function(oSettings) {
                        ...
                        $('#footer_today_total_modifier_qty').text(totalQtySoldModifier.toFixed(
                            2));
        ...
        $(document).ready(function() {
            ...
            clinic_sell_table = $('#clinic_register_report_table').DataTable({
                processing: true,
                serverSide: true,
                aaSorting: [
                    [1, 'desc']
                ...
                "ajax": {
                    "url": "/clinic/test/register/report",
                    "data": function(d) {
