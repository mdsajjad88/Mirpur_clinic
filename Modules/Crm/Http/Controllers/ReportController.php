...
class ReportController extends Controller
{
    ...
    public function index()
    {
        ...
        $call_report = $this->getDailyCallReport();

        return view('crm::reports.index')->with(compact('statuses', 'call_report'));
    }
    ...
    public function getDailyCallReport()
    {
        ...
        // Handle empty/null report_date and invalid dates
        $request_date = request()->get('report_date');
        $report_date = (!empty($request_date) && Carbon::hasFormat($request_date, 'Y-m-d')) 
            ? $request_date 
            : $today;

        $today_total_calls = CrmCallLog::where('business_id', $business_id)
            ->whereDate('created_at', Carbon::today()->format('Y-m-d'))->count();

        // Base query for CrmCallLog
        $base_call_query = CrmCallLog::where('business_id', $business_id)
            ->whereDate('created_at', $report_date);
        

        // Total calls
        $total_calls = (clone $base_call_query)->count();

        // Inbound calls
        $inbound_calls = (clone $base_call_query)
            ->where('call_type', 'inbound')
            ->count();

        // Outbound calls
        $outbound_calls = (clone $base_call_query)
            ->where('call_type', 'outbound')
            ->count();

       // Calculate call durations
        $avg_call_duration_seconds = (clone $base_call_query)
            ->whereNotNull('end_time')
            ->avg(DB::raw('TIMESTAMPDIFF(SECOND, start_time, end_time)'));
        
        $total_call_duration_seconds = (clone $base_call_query)
            ->whereNotNull('end_time')
            ->sum(DB::raw('TIMESTAMPDIFF(SECOND, start_time, end_time)'));
        
        // Convert to readable formats
        $avg_call_duration = $avg_call_duration_seconds ? gmdate('i:s', $avg_call_duration_seconds) : '0:00';
        $total_call_duration = $total_call_duration_seconds ? round($total_call_duration_seconds / 3600, 2) : 0;

        // Appointments
        $appointments = PatientAppointmentRequ::whereDate('created_at', $report_date)
            ->select(
                DB::raw('COUNT(*) as total_appointments'),
                DB::raw('SUM(CASE WHEN type = "New" THEN 1 ELSE 0 END) as new_patients'),
                DB::raw('SUM(CASE WHEN type = "Followup" THEN 1 ELSE 0 END) as followup_patients'),
                DB::raw('SUM(CASE WHEN type = "Old" THEN 1 ELSE 0 END) as old_patients'),
                DB::raw('SUM(CASE WHEN type = "Therapy" THEN 1 ELSE 0 END) as therapy_appointments')
            )
            ->first();

        $today_appointments = PatientAppointmentRequ::whereDate('request_date', $today)->count();

        $today_appointment_taken = PatientAppointmentRequ::whereDate('created_at', $today)->count();

        // Next day appointments
        $next_day = Carbon::parse($today)->addDay()->format('Y-m-d');
        $next_day_appointments = PatientAppointmentRequ::whereDate('request_date', $next_day)
            ->count();

        // Prepare the report
        $report = [
            'report_date' => Carbon::parse($report_date)->format('d/m/Y'),
            'generated_at' => Carbon::now()->format('d/m/Y h:i A'),
            'today_total_calls' => $today_total_calls,
            'total_calls' => $total_calls,
            'inbound_calls' => $inbound_calls,
            'outbound_calls' => $outbound_calls,
            'today_appointment_taken' => $today_appointment_taken,
            'today_appointments' => $today_appointments,
            'total_appointments' => $appointments->total_appointments ?? 0,
            'new_patients' => $appointments->new_patients ?? 0,
            'followup_patients' => $appointments->followup_patients ?? 0,
            'old_patients' => $appointments->old_patients ?? 0,
            'therapy_appointments' => $appointments->therapy_appointments ?? 0,
        ...
        if (request()->ajax()) {
            return response()->json([
                'html' => view('crm::reports.partials.call_report', ['call_report' => $report])->render()
            ]);
        }
