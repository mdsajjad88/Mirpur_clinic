<script type="text/javascript">
    $(document).ready(function(){
        ...
        $('#print-report-btn').click(function() {
            var printTitle = '<h3 class="text-center">@lang("crm::lang.daily_call_report")</h3>';
            var printSection = $('.report_print_section').html();
            var originalContent = $('body').html();
            $('body').html(printTitle + ' ' + printSection);
            window.print();
            $('body').html(originalContent);
        });

        $('#report_date_filter').change(function() {
            $('#report-filter-form').submit();
        });

        // AJAX filter for daily call report - full section reload
        $('#report-filter-form').on('submit', function(e) {
            e.preventDefault();
            
            var report_date = $('#report_date_filter').val();
            
            $.ajax({
                url: "{{ route('crm.daily_call_report') }}",
                type: "GET",
                data: {
                ...
                beforeSend: function() {
                    ...
                    $('.report_print_section').html('<div class="text-center"><i class="fa fa-spinner fa-spin fa-3x"></i></div>');
                },
                success: function(response) {
                    // Replace the entire section with the new HTML
                    $('.report_print_section').html(response.html);
                },
                error: function(xhr) {
