...
class CrmUtil extends Util
{
    public function addFollowUp($input, $user)
    {
        ...
        $input['schedule_type'] = ! empty($input['schedule_type']) ? $input['schedule_type'] : 'call';
        ...
        $assigned_user = !empty($input['user_id']) ? $input['user_id'] : null;
        unset($input['user_id']);
    ...
    public function addRecursiveFollowUp($input, $user)
    {
        ...
        $input['followup_category_id'] = $input['followup_category_id'];
        $assigned_user = empty($input['user_id']) ? null : $input['user_id'];
        unset($input['user_id']);
    ...
    public function addAdvanceFollowUp($input, $user)
    {
        ...
        foreach ($follow_ups as $key => $value) {
            ...
            $input['user_id'] = empty($value['user_id']) ? null : $value['user_id'];
