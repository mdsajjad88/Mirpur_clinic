...
class ProductionController extends Controller
{
    ...
    public function index()
    {
        ...
        if (request()->ajax()) {
            $productions = Transaction::join(
                ...
                ->join('users as us', 'us.id', '=', 'transactions.created_by')
            ...
            return Datatables::of($productions)
                ->addColumn('action', function ($row) {
                    ...
                    // $html .= ' <button data-href="' . action([\Modules\Manufacturing\Http\Controllers\ProductionController::class, 'destroyall'], [$row->id]) . '" class="delete-production btn btn-xs btn-danger"><i class="fa fa-trash"></i> ' . __('messages.delete') . '</button>';
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $transaction_data['final_total'] = $this->productUtil->num_uf($mfg->final_price);
            ...
            $purchase_line_data['lot_number'] = $this->generateLotNumber($variation->product_id);
            ...
            // Transform the collection into the required array format
            $ingredients = $ingredientsFromDB->mapWithKeys(function ($ingredient) {
                return [
                    $ingredient->id => [
                        ...
                        'mfg_waste_percent' => $ingredient->mfg_waste_percent,
                    ]
                ];
            })->toArray();
    ...
    public function destroyall($id)
    {
        ...
        try {
            ...
            // Get the production purchase transaction to delete
            $production_purchase = Transaction::where('business_id', $business_id)
                ->where('type', 'production_purchase')
                ->with('purchase_lines')
                ->findOrFail($id);

            // Get the corresponding production sell transaction
            $production_sell = Transaction::where('mfg_parent_production_purchase_id', $production_purchase->id)
                ->where('type', 'production_sell')
                ->with('sell_lines')
                ->first();

            // Reverse stock adjustments in production sell lines
            if ($production_purchase) {
                $transaction_id = $production_purchase->id;
                // Check if return exists for this transaction
                if ($this->transactionUtil->isReturnExist($transaction_id)) {
                    $output = [
                        'success' => false,
                ...
                $transaction = Transaction::where('id', $transaction_id)
                    ->where('business_id', $business_id)
                    ->whereIn('type', ['production_sell'])
                    ->with(['sell_lines', 'payment_lines'])
                    ->first();

                if (!empty($transaction)) {

                    // Handle non-draft transactions
                    ...
                    $sell_lines = TransactionSellLine::whereIn('id', $deleted_sell_lines_ids)->get();
                    $products = [];

                    // Loop through each sell line and map to the $products array
                    foreach ($sell_lines as $sell_line) {
                        $products[] = [
                    ...
                    $data = [
                        ...
                        'adjustment_sign' => 'minus',
                        'additional_notes' => 'This Product has from Productions',
                        'total_amount_recovered' => 00,
                        'final_total' => 00,
                        'ref_no' => null,
                    ];

                    $this->productionAdjustment($products, $data);
                }
            }
            ...
            DB::rollBack();
            // Handle exception here (e.g., log or display an error message)
            $output = ['success' => 0, 'msg' => __('messages.something_went_wrong')];
        }

        return redirect()->action([\Modules\Manufacturing\Http\Controllers\ProductionController::class, 'index'])->with('status', $output);
    }
    public function productionAdjustment($products, $data)
    {
