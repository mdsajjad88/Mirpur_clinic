...
class Category extends Model
{
     ...
     * @return array
     */
    public static function forDropdown($business_id, $type, $life_stage = null)
    {
        ...
        if (! is_null($life_stage)) {
            $categories->where('short_code', $life_stage);
        }
    ...
    public static function forDropdownClinic($business_id, $type)
    {
        $categories = Category::where('business_id', $business_id)
                            ->where('parent_id', 0)
                            ->whereIn('category_type', $type)
                            ->select(DB::raw('IF(short_code IS NOT NULL, CONCAT(name, "-", short_code), name) as name'), 'id')
                            ->orderBy('name', 'asc')
                            ->get();
