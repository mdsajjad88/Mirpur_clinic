...
class BusinessController extends Controller
{
    ...
    */
    ...
    public function getBusinessSettings()
    {
        ...
        $category_dropdown = Category::forDropdown($business_id, 'product');
        $dropdown_disabled = Product::select('category_id', \DB::raw('count(*) as total'))
        ...
        if (!empty($common_settings['contact_id'])) {
            ...
            $contactName = $contact->supplier_business_name ?? $contact->name;
        ...
        $diseases = Problem::pluck('name', 'id')->unique();
        $consultations = Product::where('product_type', 'consultation')->pluck('name', 'id');   
        $call_campaigns = CallCampaign::pluck('name', 'id')->toArray();
    ...
    public function postBusinessSettings(Request $request)
    {
        ...
        try {
            ...
            if (isset($common_settings['intake_form_disease']) && is_array($common_settings['intake_form_disease'])) {
                $common_settings['intake_form_disease'] = json_encode($common_settings['intake_form_disease']);
            }
            ...
            $selectedCategories = $request->input('disable_category_woocommerce', []);

            // Log the received categories
            Log::info('Received categories for WooCommerce sync:', [
                'categories' => $selectedCategories,
                'user_id' => $request->user()->id, // Log the user ID if authenticated
            ]);

            if (!empty($selectedCategories)) {
                // Set woocommerce_disable_sync to 0 for selected categories
                $updatedCount = Product::whereIn('category_id', $selectedCategories)
                    ->update(['woocommerce_disable_sync' => 1]);

                // Set woocommerce_disable_sync to 1 for all other categories
                $allCategories = Product::pluck('category_id')->toArray();
                $otherCategories = array_diff($allCategories, $selectedCategories);

                $updatedOtherCount = Product::whereIn('category_id', $otherCategories)
                    ->update(['woocommerce_disable_sync' => 0]);

                // Log the number of products updated
                // Log::info('Updated products for WooCommerce sync.', [
                //     'updated_count' => $updatedCount,
                //     'other_updated_count' => $updatedOtherCount,
                //     'categories' => $selectedCategories,
                // ]);
            } else {
                Log::info('No categories selected for WooCommerce sync. Setting all to 1.');

                // If no categories selected, set all to 1
                // $updatedAllCount = Product::query()->update(['woocommerce_disable_sync' => 0]);

                // Log::info('All products updated to disable sync.', [
                //     'updated_count' => $updatedAllCount,
                // ]);
            }
            $output = [
                'success' => 1,
    ...
    public function testSmsConfiguration(Request $request)
    {
        try {
            ...
            $mobile_numbers_array = explode(',', $sms_settings['test_number']);
            $results = []; // To store results for each number

            foreach ($mobile_numbers_array as $mobile_number) {
                $mobile_number = trim($mobile_number); // Clean up whitespace
                if (!empty($mobile_number)) {
                    ...
                    if ($response->getStatusCode() == 200) {
                        $results[] = [
                            'success' => 1,
                            'msg' => 'SMS sent successfully to ' . $mobile_number,
                        ];
                    } else {
                        $results[] = [
                            'success' => 0,
                            'msg' => 'Failed to send SMS to ' . $mobile_number,
                        ];
                    }
                } else {
                    $results[] = [
                        'success' => 0,
                        'msg' => __('lang_v1.test_number_is_required') . ' for ' . $mobile_number,
                    ];
                }
            }
        } catch (\Exception $e) {
        ...
        // Return all results for each number
        return $results;
    }
}
