...
class CheckController extends Controller
{
    ...
    public function randomCheck(Request $request)
    {
        ...
        try {
            if ($request->ajax()) {
                ...
                $random_checks = RandomCheck::where('random_checks.check_type', 'shop')
            ...
            $enable_reconciliation = $common_settings['enable_reconciliation'] ?? '';

            return view('random_check.index')->with(compact('business_locations', 'categories', 'enable_reconciliation'));
        } catch (\Exception $e) {
    ...
    public function archivedRandomCheck(Request $request)
    {
        ...
        try {
            if ($request->ajax()) {
                // Aggregate the sum of physical_count for each check
                $random_checks = RandomCheck::onlyTrashed()->where('random_checks.check_type', 'shop')
                    ->leftJoin('random_check_details', 'random_check_details.random_check_id', '=', 'random_checks.id')
    ...
    public function reconciliationCheck(Request $request)
    {
        if (!auth()->user()->can('stock_audit.create')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        $common_settings = $business->common_settings;
        // $exclude_period = $common_settings['exclude_period'] ?? '12';
        $location_id = $common_settings['reconciliation_location'];
        // Get the latest reconciliation check date
        $lastReconciler = RandomCheck::where('check_type', 'reconciliation')->latest()->first();
        $lastReconcilerDate = optional($lastReconciler)->created_at;
        ...
        $query = Product::with(['media'])
            ...
            ->join('variations as v', 'v.product_id', '=', 'products.id')
            ->leftJoin('variation_location_details as vld', 'vld.variation_id', '=', 'v.id')
            ->join('mfg_recipe_ingredients as ri', 'ri.variation_id', '=', 'v.id')
            ->join('transaction_sell_lines as tsl', 'tsl.variation_id', '=', 'v.id')
            ->whereNull('products.deleted_at')
            ->where('products.business_id', $business_id)
            ->whereBetween('tsl.created_at', [$lastReconcilerDate, now()])
            ->where(function ($query) {
                $query->where('tsl.mfg_ingredient_group_id', '=', 0)
                      ->orWhereNotNull('tsl.mfg_ingredient_group_id');
            })
            ->where('vld.location_id', '=', $location_id)
            ->where('products.type', '!=', 'modifier')
            ->Active();

        // Fetch products without category filtering
        $products = $query->select(
            ...
            'v.id as variation_id',
            'vld.location_id',
            DB::raw('SUM(tsl.quantity) as quantity_deducted'),
            DB::raw('SUM(vld.qty_available) as current_stock')
        )
        ->groupBy('products.id')
        // ->inRandomOrder()
        ...
        $business_locations = BusinessLocation::forDropdown($business_id, false);
        return view('random_check.reconciliation', compact('products', 'business_locations'));
    }
    ...
    public function reconciliationStore(Request $request)
    {
        ...
        try {
            ...
            $reconciliation = new RandomCheck(); // Use new to create an instance
            $reconciliation->check_no = 'Reconcile-' . $check_no;
            $reconciliation->check_type = 'reconciliation';
            $reconciliation->checked_by = auth()->id(); // Assuming you're tracking who created the reconciliation
            $reconciliation->comment = $request->input('comment') ?? '<i>no comments</i>'; // Overall comment
            $reconciliation->save();

            // Loop through the products and create reconciliation details
            foreach ($request->input('products') as $productData) {
                $reconciliationDetail = new RandomCheckDetail(); // Use new to create an instance
                $reconciliationDetail->random_check_id = $reconciliation->id;
                $reconciliationDetail->product_id = $productData['product_id'];
                $reconciliationDetail->sku = $productData['sku'];
                $reconciliationDetail->brand_name = $productData['brand'] ?? 'No Brand';
                $reconciliationDetail->category_name = $productData['category_name'] ?? 'Uncategorized';
                $reconciliationDetail->product_name = $productData['product'];
                $reconciliationDetail->current_stock = $productData['current_stock'] ?? 0; // Use current stock from the product array
                $reconciliationDetail->physical_count = $productData['physical_count'];
                $reconciliationDetail->location_id = $productData['location_id'];
                $reconciliationDetail->comment = $productData['comment'] ?? null; // Comment for each product
                $reconciliationDetail->save(); // Save the detail instance
            }

            // Commit the transaction
            DB::commit();

            return redirect()->route('random.reconciliation')->with('success', 'Reconciliation saved successfully.');
        } catch (\Exception $e) {
            // Rollback the transaction if anything goes wrong
            DB::rollBack();
            // Log the error for debugging
            Log::error('Reconciliation Store Error: ' . $e->getMessage());
            return redirect()->back()->with('error', 'An error occurred while saving the reconciliation.');
        }
    }
    ...
    public function checkEdit($id)
    {
        ...
        // Set location to null if location_id is not set in randomCheckDetail
        $location = null;
        if ($randomCheckDetail && $randomCheckDetail->location_id) {
            $location = BusinessLocation::find($randomCheckDetail->location_id);
        }
    ...
    public function reconciliation(Request $request)
    {
        ...
        try {
            if ($request->ajax()) {
                // Aggregate the sum of physical_count for each check
                $random_checks = RandomCheck::where('random_checks.check_type', 'reconciliation')
                ->leftJoin('random_check_details', 'random_check_details.random_check_id', '=', 'random_checks.id')
                    ->leftJoin('users as checked_by_user', 'random_checks.checked_by', '=', 'checked_by_user.id')
                    ->leftJoin('users as modified_by_user', 'random_checks.modified_by', '=', 'modified_by_user.id')
                    ->leftJoin('business_locations', 'random_check_details.location_id', '=', 'business_locations.id')
