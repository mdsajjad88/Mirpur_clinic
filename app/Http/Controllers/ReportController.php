...
class ReportController extends Controller
{
    ...
    public function getRegisterReport(Request $request)
    {
        ...
        $clinic_location_id = $common_settings['clinic_location'] ?? null;
        ...
        if ($request->ajax()) {
            ...
            $registers = CashRegister::leftjoin(
                ...
                ->whereNot('cash_registers.location_id', $clinic_location_id);
            ...
            Log::info(['Registering' => request()->input('user_id')]); 
            if (!empty(request()->input('user_id'))) {
                $user_ids = request()->input('user_id'); // Array of user IDs
                $registers->whereIn('cash_registers.user_id', $user_ids);
            }                      
            ...
            if (!empty(request()->input('status'))) {
                ...
                if (!empty($start_date) && !empty($end_date)) {
                    if ($status == 'close') {
                        $registers->whereDate('cash_registers.closed_at', '>=', $start_date)
                                  ->whereDate('cash_registers.closed_at', '<=', $end_date);
                    } elseif ($status == 'open') {
                        $registers->whereDate('cash_registers.created_at', '>=', $start_date)
                                  ->whereDate('cash_registers.created_at', '<=', $end_date)
                                  ->whereNull('cash_registers.closed_at');
                    }
                }
            } else {
                // Handle cases where no status is specified
                $registers->where(function ($query) use ($start_date, $end_date) {
                    $query->where(function ($subQuery) use ($start_date, $end_date) {
                        // Records where `closed_at` matches but `created_at` does not
                        $subQuery->whereDate('cash_registers.closed_at', '>=', $start_date)
                                 ->whereDate('cash_registers.closed_at', '<=', $end_date)
                                 ->where(function ($nestedQuery) use ($start_date, $end_date) {
                                     $nestedQuery->whereDate('cash_registers.created_at', '<', $start_date)
                                                 ->orWhereDate('cash_registers.created_at', '>', $end_date);
                                 });
                    })->orWhere(function ($subQuery) use ($start_date, $end_date) {
                        // Records where `created_at` matches and `closed_at` is `NULL`
                        $subQuery->whereDate('cash_registers.created_at', '>=', $start_date)
                                 ->whereDate('cash_registers.created_at', '<=', $end_date)
                                 ->whereNull('cash_registers.closed_at');
                    })->orWhere(function ($subQuery) use ($start_date, $end_date) {
                        // Include records where both `created_at` and `closed_at` are in the range
                        $subQuery->whereDate('cash_registers.created_at', '>=', $start_date)
                                 ->whereDate('cash_registers.created_at', '<=', $end_date)
                                 ->whereDate('cash_registers.closed_at', '>=', $start_date)
                                 ->whereDate('cash_registers.closed_at', '<=', $end_date);
                    });
                });
            }
            ...
            // Sort by opening time (created_at) in descending order
            $registers->orderBy('cash_registers.created_at', 'desc');
            ...
            $registers->select(
                ...
                DB::raw("SUM(IF(transaction_type='sell', amount, 0)) as total_sales")
            ...
            return Datatables::of($registers)
                ...
                ->addColumn('total', function ($row) {
                    $total = $row->total_card_payment + $row->total_cash_payment + $row->total_advance_payment + $row->total_custom_pay_1 + $row->total_custom_pay_2 + $row->total_custom_pay_3 + $row->total_custom_pay_4 + $row->total_custom_pay_5 + $row->total_custom_pay_6 + $row->total_custom_pay_7 ;
                ...
                ->addColumn('credit_sales', function ($row) {
                    $close_time = ! empty($row->closed_at) ? $row->closed_at : Carbon::now()->toDateTimeString();
                    $transaction_details = Transaction::where('transactions.created_by', $row->user_id)
                        ->whereBetween('transactions.created_at', [$row->created_at, $close_time])
                    ...
                    // Calculate credit sales: total_sales - total_sale from register details
                    $credit_sales = $transaction_details->total_sales - $register_details->total_sale;
    
                    return '<span data-orig-value="' . $credit_sales . '">' . $this->transactionUtil->num_f($credit_sales, true) . '</span>';
                })                                              
                ->rawColumns(['action', 'user_name', 'total_card_payment', 'total_cash_payment','total_advance_payment', 'total_custom_pay_1','total', 'credit_sales'])
                ->make(true);
        }
    ...
    public function getClinicRegisterReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            // $registers = $this->transactionUtil->registerReport($business_id, $start_date, $end_date);
            $registers = CashRegister::leftjoin(
                'cash_register_transactions as ct',
                'ct.cash_register_id',
                '=',
                ...
                ->where('cash_registers.business_id', $business_id)
                ->where('cash_registers.location_id', $clinic_location_id);
            ...
            return Datatables::of($registers)
                ...
                ->addColumn('action', '<button type="button" data-href="{{action(\'App\Http\Controllers\CashRegisterController@clinicShow\', [$id])}}" class="btn btn-xs btn-info btn-modal" 
                    data-container=".view_register"><i class="fas fa-eye" aria-hidden="true"></i> @lang("messages.view")</button> @if($status != "close" && auth()->user()->can("close_cash_register"))<button type="button" data-href="{{action(\'App\Http\Controllers\CashRegisterController@getClinicCloseRegister\', [$id])}}" class="btn btn-xs btn-danger btn-modal" 
                        data-container=".view_register"><i class="fas fa-window-close"></i> @lang("messages.close")</button> @endif')
                ->filterColumn('user_name', function ($query, $keyword) {
                    $query->whereRaw("CONCAT(COALESCE(surname, ''), ' ', COALESCE(first_name, ''), ' ', COALESCE(last_name, ''), '<br>', COALESCE(u.email, '')) like ?", ["%{$keyword}%"]);
                })
        ...
        return view('clinic::cash_register.index')
            ->with(compact('users', 'payment_types'));
    }
    ...
    { 
        Log::info(auth()->user()->can('expiry_report.view'));

        if (!auth()->user()->can('stock_report.view') || !auth()->user()->can('expiry_report.view')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        if ($request->ajax()) {
            ...
            $report = $query->select(
                ...
                'exp_date',
                'u.short_name as unit',
                DB::raw('SUM(COALESCE(quantity, 0) - COALESCE(quantity_sold, 0) - COALESCE(quantity_adjusted, 0) - COALESCE(quantity_returned, 0) - po_quantity_purchased) as stock_left'),
                't.ref_no',
            ...
            return Datatables::of($report)
                ->editColumn('product', function ($row) {
                    if ($row->product_type == 'variable') {
                        return $row->product . ' - ' .
                            $row->product_variation . ' - ' . $row->variation . ' (' . $row->sub_sku . ')'. $row->pid;
                    } else {
                        return $row->product . ' (' . $row->sku . ')'. $row->pid;
                    }
                })
    ...
    public function getproductQuantityReport(Request $request)
    {
        if (!auth()->user()->can('purchase_n_sell_report.view')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        return view('report.product_qty_report')
            ->with(compact(
                'business_locations',
                'categories',
                'brands',
    ...
    public function getproductSellGroupedReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            ->select(
                ...
                DB::raw('(SELECT SUM(vld.qty_available) FROM variation_location_details as vld WHERE vld.variation_id = v.id) as current_stock') // Alias for current stock
            )
            ->groupBy('v.id')
    ...
    {       
        ...
        if ($request->ajax()) {
            ...
            $end_date = $request->get('end_date');
            if (empty($start_date)) {
                $start_date = date('Y-m-d'); // Format as 'YYYY-MM-DD'
            }
            // Check if $end_date is empty, if so, set it to the date 12 months ago
            if (empty($end_date)) {
                $end_date = date('Y-m-d', strtotime('-12 months')); // Format as 'YYYY-MM-DD'
            }
            $permitted_locations = auth()->user()->permitted_locations();
            $subquery = TransactionSellLine::join(
                'transactions as t',
                'transaction_sell_lines.transaction_id',
                '=',
                't.id'
                ...
                ->where('t.business_id', $business_id);
                if (!empty($start_date) && !empty($end_date)) {
                    $subquery->whereDate('transaction_sell_lines.created_at', '>=', $start_date)
                        ->whereDate('transaction_sell_lines.created_at', '<=', $end_date);
                }
              $sold_products_subquery=  $subquery->groupBy('v.id')
                ->pluck('v.id') // Retrieves only the product IDs
                ->toArray(); // Converts the result into an array
            // dd($sold_products_subquery); //
            Log::info('sold_products_subquery ', [
                'sold_products_subquery' => count($sold_products_subquery),
            ...
            $query = Product::leftJoin('brands', 'products.brand_id', '=', 'brands.id')
                ...
                ->leftJoin('transaction_sell_lines as tsl', 'v.id', '=', 'tsl.variation_id')
                ...
                ->whereNull('v.deleted_at')
                ->whereNotIn('v.id', $sold_products_subquery)
                ->where('products.business_id', $business_id)
            ...
            return Datatables::of($query)
                ->editColumn('product_name', function ($row) {
                   ...
                   $html = '<a target="_blank" href="' . action([\App\Http\Controllers\ProductController::class, 'productStockHistory'], [$row->id]) . '">' . $product_name . '</a>';
                    // '<a href="'.action([\App\Http\Controllers\ProductController::class, 'productStockHistory'], [$row->id]).'"><i class="fas fa-history"></i> '.$html.'</a>';
                    return $html;
                })
                ->editColumn('transaction_date', '10/12/2024')
                ->editColumn('total_qty_sold', function ($row) {
                    return 0;
                })
                ->editColumn('current_stock', function ($row) use ($start_date) {
                ...
                ->editColumn('transaction_date', 'Transaction Date')->addColumn('mass_delete', function ($row) {
                ...
                ->rawColumns(['product_name', 'sub_sku','mass_delete', 'current_stock', 'subtotal', 'total_qty_sold'])
                ->make(true);
        }
    }
    
    public function getproductQuantityReportInactive(Request $request)
    {
        $business_id = $request->session()->get('user.business_id');
        $location_id = $request->get('location_id', null);
        ...
        if ($request->ajax()) {
            ...
            $permitted_locations = auth()->user()->permitted_locations();
            $purchaseQuery =PurchaseLine::join(
                'transactions as t',
                'purchase_lines.transaction_id',
                '=',
                't.id'
                ...
                ->where('t.type', 'purchase');
                if (!empty($start_date) && !empty($end_date)) {
                    $purchaseQuery->whereDate('purchase_lines.created_at', '<=', $start_date);
                }
            $subquery = TransactionSellLine::join(
                'transactions as t',
                'transaction_sell_lines.transaction_id',
            ...
            $sold_products_subquery =  $subquery->groupBy('v.id')
                ->pluck('v.id') 
                ->toArray();
            $purchase_products_subquery =  $purchaseQuery->groupBy('v.id')
                ->pluck('v.id') 
                ->toArray();
            $merged_product_ids = array_merge($sold_products_subquery, $purchase_products_subquery);
            ...
            $query = Product::leftJoin('brands', 'products.brand_id', '=', 'brands.id')
                ...
                ->whereNull('v.deleted_at')
                ->whereIn('v.id', $purchase_products_subquery)
                ->whereNotIn('v.id', $sold_products_subquery)
                ->where('products.business_id', $business_id)
                ->where('products.type', '!=', 'modifier');
            ...
            $brand_id = request()->get('brand_id', null);
            if (! empty($brand_id)) {
                $products->where('brands.id', $brand_id);
            }
            ...
            return Datatables::of($products)
                ...
                ->rawColumns(['product', 'sub_sku', 'mass_delete', 'current_stock', 'subtotal', 'total_qty_sold','category'])
                ->make(true);
        }
    }
    /**
        ...
        if (request()->ajax()) {
            ...
            return Datatables::of($activities)
                ...
                ->addColumn('subject_type', function ($row) use ($transaction_types) {
                    ...
                    } elseif (($row->subject_type == \App\SellingPriceGroup::class)) {
                        $subject_type = __('lang_v1.selling_price_group');
                    } elseif (($row->subject_type == \App\VariationGroupPrice::class)) {
                        $subject_type = __('lang_v1.variation_group_price');
                    } elseif (($row->subject_type == \Modules\Clinic\Entities\DoctorProfile::class)) {
                        $subject_type = 'Doctor Profile';
                    } elseif (($row->subject_type == \Modules\Clinic\Entities\PatientSessionInfo::class)) {
                        $subject_type = 'Patient Subscription';
                    } 
                    elseif (($row->subject_type == \Modules\Clinic\Entities\PatientProfile::class)) {
                        $subject_type = 'Patient Profile';
                    }
                    elseif (($row->subject_type == \Modules\Clinic\Entities\DoctorSL::class)) {
                        $subject_type = 'Doctor Serial';
                    }
                    elseif (($row->subject_type == \Modules\Clinic\Entities\Prescription::class)) {
                        $subject_type = 'Prescription';
                    }
                    elseif (($row->subject_type == \Modules\Clinic\Entities\PrescribeMedicine::class)) {
                        $subject_type = 'Prescription Medicine';
                    }
                    elseif (($row->subject_type == \Modules\Clinic\Entities\PrescribeTest::class)) {
                        $subject_type = 'Prescription Test';
                    }
                    elseif (($row->subject_type == \Modules\Clinic\Entities\PrescribeTherapie::class)) {
                        $subject_type = 'Prescription Therapy';
                    }
                ...
                ->addColumn('note', function ($row) use ($statuses, $shipping_statuses) {
                    ...
                    if(!empty($row->properties['medicine_ids'])){
                        $html .= __('Medicine IDs') . ': ' . $row->properties['medicine_ids'] . '<br>';
                    }
                   
                    if(!empty($row->properties['old_medicine_count'])){
                        $html .= __('Old Medicine Count') . ': ' . $row->properties['old_medicine_count'] . '<br>';
                    }
                    if(!empty($row->properties['new_medicine_count'])){
                        $html .= __('New Medicine Count') . ': ' . $row->properties['new_medicine_count'] . '<br>';
                    }
                    if(!empty($row->properties['therapy_ids'])){
                        $html .= __('Therapy IDs') . ': ' . $row->properties['therapy_ids'] . '<br>';
                    }
                   
                    if(!empty($row->properties['old_therapy_count'])){
                        $html .= __('Old Therapy Count') . ': ' . $row->properties['old_therapy_count'] . '<br>';
                    }
                    if(!empty($row->properties['new_therapy_count'])){
                        $html .= __('New Therapy Count') . ': ' . $row->properties['new_therapy_count'] . '<br>';
                    }
                    
                    if(!empty($row->properties['test_ids'])){
                        $html .= __('Test IDs') . ': ' . $row->properties['test_ids'] . '<br>';
                    }
                   
                    if(!empty($row->properties['old_test_count'])){
                        $html .= __('Old Test Count') . ': ' . $row->properties['old_test_count'] . '<br>';
                    }
                    if(!empty($row->properties['new_test_count'])){
                        $html .= __('New Test Count') . ': ' . $row->properties['new_test_count'] . '<br>';
                    }
                    ...
                    $html .= '<div class="audit-data-container">'; // Start the unique container for side-by-side layout

                    // Check if 'old' data is available
                    if (!empty($row->getExtraProperty('before'))) {
                        $oldData = $row->getExtraProperty('before');
                        
                        // Check if 'old' is an array or object
                        if (is_array($oldData) || is_object($oldData)) {
                            $html .= '<div class="data-section"><strong>' . __('Before') . ':</strong>';
                            
                            // Iterate through each key-value pair in the 'old' data
                            $html .= '<ul class="data-list">';
                            foreach ($oldData as $key => $value) {
                                $html .= '<li><strong>' . ucfirst(str_replace('_', ' ', $key)) . ':</strong> ' . htmlspecialchars($value) . '</li>';
                            }
                            $html .= '</ul></div>'; // Close the list
                        }
                    }

                    // Check if 'attributes' data is available
                    if (!empty($row->getExtraProperty('after'))) {
                        $attributesData = $row->getExtraProperty('after');
                        
                        // Check if 'attributes' is an array or object
                        if (is_array($attributesData) || is_object($attributesData)) {
                            $html .= '<div class="data-section"><strong>' . __('After') . ':</strong>';
                            
                            // Iterate through each key-value pair in the 'attributes' data
                            $html .= '<ul class="data-list">';
                    ...
                    $html .= '</div>'; // End the unique container for side-by-side layout

                    // Check if 'ip_address' is available
                    if (!empty($row->getExtraProperty('ip_address'))) {
                        $html .= '<div class="data-section"><strong>' . __('IP Address') . ':</strong> ' . htmlspecialchars($row->getExtraProperty('ip_address')) . '</div>';
                        }
    ...
    public function ModifiersSellReport(Request $request)
    {
        $business_id = $request->session()->get('user.business_id');
        $location_id = $request->get('location_id', null);
        $is_woocommerce = $this->moduleUtil->isModuleInstalled('Woocommerce');
        ...
        if ($request->ajax()) {
            ...
            $query = TransactionSellLine::join('transactions as t', 'transaction_sell_lines.transaction_id', '=', 't.id')
                ...
                ->where('t.type', 'sell')
                ->where('t.status', 'final')
                ->where('transaction_sell_lines.children_type', 'modifier') // Filter for products of type 'modifier'
                ->select(
                    'p.name as product_name',
                    'p.enable_stock',
                    'cat.name as category_name',
                    ...
                    'u.short_name as unit',
                    DB::raw('SUM((transaction_sell_lines.quantity - transaction_sell_lines.quantity_returned) * transaction_sell_lines.unit_price_inc_tax) as subtotal'),
                    DB::raw('(SELECT SUM(vld.qty_available) FROM variation_location_details as vld WHERE vld.variation_id = v.id) as current_stock')
                )
                ->groupBy('v.id');
        ...
        return view('report.modifier_sell_report')->with(compact(
            'rack_enabled',
            'categories',
            'brands',
            'units',
