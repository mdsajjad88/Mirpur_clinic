...
class ProductModifierSetController extends Controller
{
    ...
    public function edit($id)
    {
        if (request()->ajax()) {
            ...
            // Get the count of variations
            $variation_count = $modifer_set->variations->count();

            return view('restaurant.product_modifier_set.edit')
                ->with(compact('modifer_set', 'variation_count'));
        }
    }
    ...
    public function update($modifier_set_id, Request $request)
    {
        ...
        try {
            ...
            // Check if the modifier set exists
            if (!$modifer_set) {
                return response()->json(['success' => 0, 'msg' => __('messages.something_went_wrong')]);
            }

            // Get the products to be synced
            $products = [];
            if (!empty($input['products'])) {
                $products = $input['products'];
            }

            // Get the modifier limits for each product
            $modifier_limits = [];
            if (!empty($input['modifier_limits'])) {
                $modifier_limits = $input['modifier_limits'];
            }

            // Sync the products with their limits
            foreach ($products as $product_id) {
                $modifer_set->modifier_products()->syncWithoutDetaching([
                    $product_id => ['modifier_limit' => $modifier_limits[$product_id] ?? null]
                ]);
            }

            // Handle removed products
            if (!empty($input['removed_products'])) {
                $modifer_set->modifier_products()->detach($input['removed_products']);
            }
