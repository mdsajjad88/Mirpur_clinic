...
class AdminSidebarMenu
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        Menu::create('admin-sidebar-menu', function ($menu) {
                ...
                $menu->dropdown(
                    ...
                    function ($sub) {
                        ...
                        if (auth()->user()->can('product.edit')) {
                            $sub->url(
                                action([\App\Http\Controllers\ExpiryDateController::class, 'create']),
                                __('Edit Expiry Date'),
                                ['icon' => 'fa fas fa-plus-circle', 'active' => request()->segment(1) == 'expiry' && request()->segment(2) == 'create']
                            );
                        }
                        $sub->url(
                            action([\App\Http\Controllers\CheckController::class, 'randomCheck']),
                            __('Stock Audit'),
                            ['icon' => 'fa fas fa-shield-alt', 'active' => request()->segment(1) == 'random' && request()->segment(2) == 'random-check-index']
                        );
                        if (auth()->user()->can('product.create')) {
                            $sub->url(
                    ...
                    function ($sub) use ($common_settings) {
                        ...
                        if (auth()->user()->can('purchase.create')) {
                            $sub->url(
                                ...
                                'Stock Surplus',
                                ['icon' => 'fa fas fa-plus-circle', 'active' => request()->segment(1) == 'stock-surplus']
                            );
                        }
                ...
                $menu->dropdown(
                    ...
                    function ($sub) use ($enabled_modules, $is_admin, $pos_settings) {
                        ...
                        $sub->url(
                            ...
                            __('Demands'),
                            ['icon' => 'fa fas fa-file-alt', 'active' => request()->segment(1) == 'waitlists']
                        );
                        ...
                        if (in_array('add_sale', $enabled_modules) && ($is_admin || auth()->user()->hasAnyPermission(['draft.view_all', 'draft.view_own']))) {
                            $sub->url(
                                action([\App\Http\Controllers\SellController::class, 'getEcommerce']),
                                __('lang_v1.list_ecoomerce_order'),
                                ['icon' => 'fa fas fa-pen-square', 'active' => request()->segment(1) == 'sells' && request()->segment(2) == 'ecommerce']
                            );
                        }
                        if (in_array('add_sale', $enabled_modules) && auth()->user()->can('direct_sell.access')) {
                            $sub->url(
            ...
            if (in_array('stock_adjustment', $enabled_modules) && (auth()->user()->can('stock_adjustment.view') || auth()->user()->can('stock_adjustment.create'))) {
                $menu->dropdown(
                ...
                $menu->dropdown( 
                    ...
                    function ($sub) use ($enabled_modules, $is_admin) {
                        ...
                        if (auth()->user()->can('purchase_n_sell_report.view')) {
                            $sub->url(
                                action([\App\Http\Controllers\ReportController::class, 'ModifiersSellReport']),
                                __('Modifiers Sell Report'),
                                ['icon' => 'fa fas fa-undo', 'active' => request()->segment(2) == 'modifiers-sell-report']
                            );
                        }
                        ...
                        if (auth()->user()->can('purchase_n_sell_report.view')) {
                            ...
                            $sub->url(
                                action([\App\Http\Controllers\ReportController::class, 'getproductQuantityReport']),
                               __('lang_v1.product_activity_report'),
                                ['icon' => 'fa fas fa-arrow-circle-up', 'active' => request()->segment(2) == 'product-qty-report']
                            );
                        ...
                        // if (auth()->user()->can('register_report.view')) {
                        //     $sub->url(
                        //         action([\App\Http\Controllers\CashRegisterController::class, 'getRegisterDetailsOverall']),
                        //         __('Accounts Report'),
                        //         ['icon' => 'fa fas fa-check-double', 'active' => request()->segment(2) == 'overall']
                        //     );
                        // }
