...
class ProductUtil extends Util
{
    ...
    public function getDetailsFromVariation($variation_id, $business_id, $location_id = null, $check_qty = true)
    {
        ...
        $product = $query->select(
            ...
            'p.product_type as product_type_for_clinic',
    ...
    public function calculateInvoiceTotal($products, $tax_id, $discount = null, $uf_number = true)
    {
        ...
        $max_discount = 0;
        $min_sell_amount = 0;
        $role_discount_type = null;

        // Determine discount for privileged users
        if (auth()->user()->can('discount_no_limit') || auth()->user()->can('superadmin') || auth()->user()->can('admin')) {
        ...
        } elseif (isset($pos_settings['discount_rules']) && isset($pos_settings['enable_discount_rules'])) {
            // Check for discount rules based on business settings
            foreach ($pos_settings['discount_rules'] as $rule) {
                ...
                $rule_discount_type = $rule['discount_type'];

                if ($output['total_before_tax'] >= $rule_min_sell_amount) {
                    $min_sell_amount = $rule_min_sell_amount;
                    $max_discount = $rule_discount_type === 'percentage'
                        ? ($rule_max_discount / 100) * $output['total_before_tax']
                        : $rule_max_discount;
                    $role_discount_type = $rule_discount_type; // Store the role discount type for comparison
                }
            }
            ...
            if (is_array($discount) && isset($discount['discount_amount'])) {
                ...
                // Validate minimum sell amount for discounts
                if ($output['total_before_tax'] < $min_sell_amount) {
                    ...
                    if ($discount['discount_type'] === 'fixed') {
                        // Ensure fixed discount does not exceed max discount
                        ...
                        $output['discount'] = $discount_amount; // Use the fixed discount amount
                    } elseif ($discount['discount_type'] === 'percentage') {
                        // Calculate percentage discount and ensure it does not exceed max discount
                        $calculated_discount = ($discount_amount / 100) * $output['total_before_tax'];
                        ...
                        $output['discount'] = $calculated_discount; // Use the calculated percentage discount
                    } 
                }
            }
        } else {
            // Default handling of discount if no rules defined
            if (is_array($discount)) {
                $discount_amount = $uf_number ? $this->num_uf($discount['discount_amount']) : $discount['discount_amount'];
                // Apply discount based on type
                if ($discount['discount_type'] === 'fixed') {
        ...
        if (!is_numeric($output['discount'])) {
            $output['discount'] = 0; // Default to 0 if not a number
        }
    ...
    private function calculateDiscount($discount, $discount_amount, $total_before_tax, $max_discount, $rule_discount_type)
    {
        if ($discount['discount_type'] === $rule_discount_type) {
            if ($discount_amount > $max_discount) {
                return [
                    'success' => 0,
                    'msg' => 'Discount exceeds the maximum allowed discount.',
    ...
    public function updateProductFromPurchase($variation_data)
    {
        ...
        if ($product->category_id == (isset($foreign_cat) ? $foreign_cat->id : null)) {
            if (($variation_details->default_purchase_price != $variation_data['pp_without_discount']) ||
                ...
                // Check variation price history
                $latest_price_history = VariationPriceHistory::where('variation_id', $variation_data['product_id'])
                    ->latest('created_at')
                    ->first();

                // Create a new price history entry if no entry is found or the price has changed
                if (!$latest_price_history || 
                    $latest_price_history->old_price != $variation_details->dpp_inc_tax || 
                    $latest_price_history->new_price != $variation_details->sell_price_inc_tax) {
                    
                    VariationPriceHistory::create([
                        'variation_id' => $variation_data['product_id'],
                        'old_price' => '$ ' . number_format($variation_details->foreign_p_price_inc_tex, 2) . 
                                    '<br>৳ ' . number_format($variation_details->dpp_inc_tax, 2) . 
                                    '<br>$⇄৳ ' . number_format($foreign_cat->description, 2),
                        'new_price' => '$ ' . number_format($variation_details->foreign_s_price_inc_tex, 2) . 
                                    '<br>৳ ' . number_format($variation_details->sell_price_inc_tax, 2) . 
                                    '<br>$⇄৳ ' . number_format($foreign_cat->description, 2),
                        'updated_by' => auth()->id(),
                        'type' => 'product',
                        'h_type' => 'Purchase',
    ...
    public function createOrUpdatePurchaseLines($transaction, $input_data, $currency_details, $enable_product_editing, $before_status = null)
    {
        ...
        foreach ($input_data as $data) {
            ...
            if (isset($data['purchase_line_id'])) {
                ...
                // Get all waitlists for the given product_id
                $waitlists = ProductWaitlist::where('product_id', $data['product_id'])->get();

                // Loop through the waitlists and update them
                foreach ($waitlists as $waitlist) {
                    $waitlist->status = 'Available';
                    $waitlist->restock_date = now();
                    $waitlist->save();
                }
    ...
    public function filterProductSale($business_id, $search_term, $location_id = null, $not_for_selling = null, $price_group_id = null, $product_types = [], $search_fields = [], $check_qty = false, $search_type = 'like')
    {
        ...
        $query->select(
                ...
                'variations.id as variation_id',
                'variations.name as variation',
                // 'VLD.qty_available as total_quantity',
                'variations.sell_price_inc_tax as selling_price',
                'variations.sub_sku',
                'U.short_name as unit',
                DB::raw('MIN(CASE 
                    WHEN (pl.quantity - (pl.quantity_sold + pl.quantity_adjusted + pl.quantity_returned + pl.mfg_quantity_used + pl.po_quantity_purchased)) > 0 
                    THEN pl.exp_date 
                    ELSE NULL 
                    END) as nearest_exp_date'
                ),
                'pl.id as purchase_line_id',
                'pl.transaction_id',
                'pl.lot_number',
                'pl.exp_date',
                DB::raw('IF(VLD.location_id = ' . ($location_id ?? 'NULL') . ', VLD.qty_available, 0) as qty_available'),
                DB::raw('(SELECT SUM(vld.qty_available) FROM variation_location_details vld WHERE vld.variation_id = variations.id) as total_quantity')
            );
    ...
    public function filterProduct($business_id, $search_term, $location_id = null, $not_for_selling = null, $price_group_id = null, $product_types = [], $search_fields = [], $check_qty = false, $search_type = 'like')
    {
        ...
        if (!empty($not_for_selling)) {
            if($not_for_selling=='2' && $pos_settings['not_for_sale']=='1'){
        ...
        // Log::info(['not_for_sall' =>$pos_settings['not_for_sale']]);
        if (! empty($price_group_id)) {
            $query->leftjoin(
                'variation_group_prices AS VGP',
                function ($join) use ($price_group_id) {
