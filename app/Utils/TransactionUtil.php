...
class TransactionUtil extends Util
{
    ...
    public function createSellTransaction($business_id, $input, $invoice_total, $user_id, $uf_data = true)
    {
        ...
        $transaction = Transaction::create([
            ...
            'reference_id' => !empty($input['reference_id']) ? $input['reference_id'] : null,
    ...
    public function updateSellTransaction($transaction_id, $business_id, $input, $invoice_total, $user_id, $uf_data = true, $change_invoice_number = true)
    {
        ...
        $update_date = [
            ...
            'additional_notes' => !empty($input['sale_note']) ? $input['sale_note'] : null,
            'appointment_date' => !empty($input['appointment_date']) ? $input['appointment_date'] : null,
            'staff_note' => !empty($input['staff_note']) ? $input['staff_note'] : null,
            'custom_field_1' => !empty($input['custom_field_1']) ? $input['custom_field_1'] : null,
            'custom_field_2' => !empty($input['custom_field_2']) ? $input['custom_field_2'] : null,
            'custom_field_3' => !empty($input['custom_field_3']) ? $input['custom_field_3'] : null,
        ...
        if (!empty($input['appointment_date'])) {
            Log::info('Appointment date' . ($input['appointment_date']));
            $update_date['appointment_date'] = $input['appointment_date'];
        }
    ...
    public function createOrUpdateSellLines($transaction, $products, $location_id, $return_deleted = false, $status_before = null, $extra_line_parameters = [], $uf_data = true)
    {
        ...
        foreach ($products as $product) {
            ...
            if (!empty($product['transaction_sell_lines_id'])) {
                ...
                Log::info('Item tax and line discount', [
                    'uf_item_tax' => $uf_item_tax,
    ...
    public function createOrUpdatePaymentLines($transaction, $payments, $business_id = null, $user_id = null, $uf_data = true, $cat_desck = null)
    {
        ...
        foreach ($payments as $payment) {
            ...
            if (!empty($payment['payment_id'])) {
                ...
                $return_value = isset($payment['is_return']) ? (int) $payment['is_return'] : 0;
                $payment_amount = isset($payment_amount) ? (float) $payment_amount : 0;
                ...
                if ($return_value === 1 && $payment_amount === 0.0) {
                    $payment_val = false;
                } else {
                    $payment_val = true;
                }

                Log::info('Payment Status:', ['payment_val' => $payment_val]);
    ...
    public function editPaymentLine($payment, $transaction = null, $uf_data = true)
    {
        ...
        $payment['amount'] = $uf_data ? $this->num_uf($payment['amount']) : $this->num_uf($payment['amount']);
     ...
     */
    // 
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type,$appointment_id=null)
    {
        $il = $invoice_layout;

        $transaction = Transaction::with('reference')->find($transaction_id);
        $transaction_type = $transaction->type;
        $formatted_sl_no = '';
        if (!empty($appointment_id)) {
            $sl_no_record = DoctorSL::where('appointment_id', $appointment_id)->first();
            if (!empty($sl_no_record)) {
                $formatted_sl_no .= $sl_no_record->sl_no;
            }
        }
        $output = [
            ...
            'customer_info_clinic' => '',
            'sub_type' => $transaction->sub_type,
            'reference' => $transaction->reference
                ? $transaction->reference->first_name . ' ' . $transaction->reference->last_name
                : '',
            'designation' => $transaction->reference && $transaction->reference->is_show_invoice == 1 && $transaction->reference->designation
                ? "(" . $transaction->reference->designation . ')'
                : '',
            'formatted_sl_no' => $formatted_sl_no,
        ...
        if ($transaction_type == 'sell_return') {
            ...
            $output['sell_method'] = $sell_method->method ?? 'Due';
        }
        ...
        $customer = Contact::with('patientProfiles')->find($transaction->contact_id);
        ...
        if ($il->show_customer == 1) {
            ...
            $output['customer_info_clinic'] .= $output['customer_name'];
            ...
            if ($receipt_printer_type != 'printer') {
                ...
                if ($customer->patientProfiles->isNotEmpty()) {
                    $firstProfile = $customer->patientProfiles->first();

                    // Check and append gender if it's not empty
                    if (!empty($firstProfile->gender)) {
                        $gender = ', ' . ucfirst($firstProfile->gender);  // Set gender if it's available
                        $output['customer_info'] .= $gender;  // Append gender to customer info
                    }

                    // Check and append age if it's not empty
                    if (!empty($firstProfile->age)) {
                        $age = ', Age: ' . $firstProfile->age;
                        $output['customer_info'] .= $age;
                    }

                    $output['customer_info_clinic'] .= $gender . $age;
                }
            }
        ...
        if ($il->show_sales_person == 1) {
            ...
            $output['sales_person'] = !empty($transaction->sales_person->username) ? $transaction->sales_person->username : '';
        }
        ...
        if (blank($il->date_time_format)) {
            ...
            $output['invoice_final_date'] = $this->format_date($transaction->appointment_date, true, $business_details);
        } else {
            $output['invoice_date'] = \Carbon::createFromFormat('Y-m-d H:i:s', $transaction->transaction_date)->format($il->date_time_format);
            $output['invoice_final_date'] = \Carbon::createFromFormat('Y-m-d H:i:s', $transaction->appointment_date)->format($il->date_time_format);
        }

        $output['transaction_date'] = $transaction->transaction_date;
        $output['invoice_final_date'] = $transaction->appointment_date;
        ...
        if (in_array($transaction_type, ['sell', 'sales_order'])) {
            ...
            foreach ($details['lines'] as $line) {
                ...
                Log::info('Discount info:', ['line_discount_uf' => $line['line_discount_uf'], 'line_quantity_uf' => $line['quantity_uf']]);
                ...
                if (!empty($payments)) {
                    foreach ($payments as $value) {
                        ...
                        if ($value['method'] == 'cash') {
                            ...
                            $payment_entry['method_id'] .= !empty($value['card_transaction_number']) ? ',TXN:' . $value['card_transaction_number'] : '';
                            ...
                            $payment_entry['method_id'] .= !empty($value['transaction_no']) ? ',TXN:' . $value['transaction_no'] : '';
                        }
        ...
        Log::info('Output ' . json_encode($output['customer_info']));
    ...
    protected function _receiptDetailsSellLines($lines, $il, $business_details)
    {
        ...
        foreach ($lines as $line) {
            ...
            $line_array = [
                ...
                'unit_price_exc_tax_uf' => $line->unit_price,
                ...
                'unit_price_inc_tax_before_discount' => $this->num_f($line->unit_price_before_discount + $line->item_tax, false, $business_details),
                'unit_price_inc_tax_before_discount_uf' => $line->unit_price_before_discount + $line->item_tax,
                'unit_price_inc_tax_after_discount' => $this->num_f($line->unit_price + $line->item_tax, false, $business_details),
                'unit_price_inc_tax_after_discount_uf' => $line->unit_price + $line->item_tax,
            ...
            // $line_array['line_discount_uf'] = $this->num_uf($line_array['unit_price_before_discount']) - $this->num_uf($line_array['unit_price_inc_tax']);
    ...
    public function getSellTotalsClinic($business_id, $start_date = null, $end_date = null, $location_id = null, $created_by = null, $permitted_locations = null) {
        // Base query for both aggregations
        $baseQuery = Transaction::where('transactions.business_id', $business_id)
            ->where('transactions.type', 'sell')
            ->where('transactions.status', 'final')
            ->whereNull('transactions.deleted_at'); // Handle soft deletes
        ...
        if (!empty($start_date) && !empty($end_date)) {
            $baseQuery->whereDate('transactions.transaction_date', '>=', $start_date)
                ->whereDate('transactions.transaction_date', '<=', $end_date);
        }

        if (empty($start_date) && !empty($end_date)) {
            $baseQuery->whereDate('transactions.transaction_date', '<=', $end_date);
        }
        ...
        if (!empty($created_by)) {
            $baseQuery->where('transactions.created_by', $created_by);
        }

        // === 1. Overall aggregated totals ===
        $sell_details = (clone $baseQuery)->selectRaw('
            COUNT(id) as count_total_bill_income,
            SUM(final_total) as total_bill_income,
            COUNT(CASE WHEN type = "sell_return" THEN id END) as count_return_refund,
            SUM(CASE WHEN type = "sell_return" THEN final_total END) as return_refund,
            SUM(final_total - tax_amount) as total_exc_tax,
            SUM(total_before_tax) as total_before_tax,
            SUM(shipping_charges) as total_shipping_charges,
            SUM(additional_expense_value_1 + additional_expense_value_2 + additional_expense_value_3 + additional_expense_value_4) as total_expense,
            SUM(CASE 
                WHEN discount_type = "fixed" THEN discount_amount
                WHEN discount_type = "percentage" THEN (discount_amount / 100) * total_before_tax
                ELSE 0
            END) as special_discount
        ')->first();

        // === 2. Per-customer aggregated details ===
        $customer_details = (clone $baseQuery)->where('transactions.discount_amount', '>', 0)
        ...
        // === 2.1. Total bill income per customer ===
        $bill_income_per_customer = (clone $baseQuery)
            ->leftJoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
            ->groupBy('transactions.id')
            ->selectRaw('
                transactions.id,
        ...
        $net_income = ($sell_details->total_bill_income ?? 0) - ($sell_details->return_refund ?? 0);

        // === 4. Return all together ===
        return [
            'total_bill_income' => $sell_details->total_bill_income ?? 0,
            'count_total_bill_income' => $sell_details->count_total_bill_income ?? 0,
            'return_refund' => $sell_details->return_refund ?? 0,
            'count_return_refund' => $sell_details->count_return_refund ?? 0,
            'special_discount' => $sell_details->special_discount ?? 0,
            'net_income' => $net_income,
            'customer_details' => $customer_details,
            'bill_income_per_customer' => $bill_income_per_customer
        ];
    }

    public function getDueIncomeClinic($business_id, $start_date = null, $end_date = null, $location_id = null, $created_by = null, $permitted_locations = null)
    {
        $query = Transaction::where('transactions.business_id', $business_id)
        ...
        // Date filter logic
        if (!empty($start_date) && !empty($end_date)) {
            $query->whereBetween(DB::raw('DATE(TP.created_at)'), [$start_date, $end_date])
                ->whereNotBetween(DB::raw('DATE(transactions.transaction_date)'), [$start_date, $end_date]);
        }
        ...
        $sell_details = $query->get(); // fetch all

        $count_due_income = $sell_details->count();
        $due_income = $sell_details->sum('amount');

        return [
            'sell_details' => $sell_details,
            'due_income' => $due_income,
            'count_due_income' => $count_due_income,
        ];
    }


    public function getDueBillClinic($business_id, $start_date = null, $end_date = null, $location_id = null, $created_by = null, $permitted_locations = null)
    {
        $query = Transaction::where('transactions.business_id', $business_id)
            ->leftJoin('transaction_payments AS TP', 'transactions.id', '=', 'TP.transaction_id')
            ->leftJoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
        ...
        $count_due_bill = $results->count();
        $due_bill = $results->sum(function ($item) {
            return $item->final_total - $item->total_paid;
        });

        return [
            'sell_details' => $results,
            'count_due_bill' => $count_due_bill,
            'due_bill' => $due_bill,
        ];
    }

    public function getCashClinic($business_id, $start_date = null, $end_date = null, $location_id = null, $created_by = null, $permitted_locations = null)
    {
        $query = Transaction::where('transactions.business_id', $business_id)
            ->leftJoin('transaction_payments AS TP', 'transactions.id', '=', 'TP.transaction_id')
            ->leftJoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
            ->where('transactions.type', 'sell')
            ->where('transactions.status', 'final')
            ->where('TP.method', 'cash') // Ensure only cash payments
            ->where('TP.is_return', 0) // Exclude returned payments
            ->select(
                'transactions.id',
                'transactions.invoice_no',
                'transactions.final_total',
        ...
        $due_income_query = Transaction::where('transactions.business_id', $business_id)
            ->leftJoin('transaction_payments AS TP', 'transactions.id', '=', 'TP.transaction_id')
            ->leftJoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
            ->where('transactions.type', 'sell')
            ->where('transactions.status', 'final')
        ...
        if (!empty($start_date) && !empty($end_date)) {
            $due_income_query->whereBetween(DB::raw('DATE(TP.created_at)'), [$start_date, $end_date])
                ->whereNotBetween(DB::raw('DATE(transactions.transaction_date)'), [$start_date, $end_date]);
            $query->whereDate('transactions.transaction_date', '>=', $start_date)
                ->whereDate('transactions.transaction_date', '<=', $end_date);
        }
        ...
        if (empty($start_date) && !empty($end_date)) {
            $query->whereDate('transactions.transaction_date', '<=', $end_date);
            $due_income_query->whereDate('transactions.transaction_date', '<=', $end_date);
        }

        // Check for permitted locations
        if (!empty($permitted_locations) && $permitted_locations != 'all') {
            $query->whereIn('transactions.location_id', $permitted_locations);
            $due_income_query->whereIn('transactions.location_id', $permitted_locations);
        }

        // Filter by location
        if (!empty($location_id)) {
            $query->where('transactions.location_id', $location_id);
            $due_income_query->where('transactions.location_id', $location_id);
        }

        // Filter by user
        if (!empty($created_by)) {
            $query->where('transactions.created_by', $created_by);
            $due_income_query->where('transactions.created_by', $created_by);
        }

        $sell_details = $query->get();
        $due_income = $due_income_query->get();
        return [
            'current_income' => $sell_details,
            'due_income' => $due_income,
            'cash_amount' => $sell_details->sum('amount') + $due_income->sum('amount'),
        ];
    }
    public function getServiceClinic($business_id, $start_date = null, $end_date = null, $location_id = null, $created_by = null, $permitted_locations = null)
    {
        $query = Transaction::where('transactions.business_id', $business_id)
            ->where('transactions.type', 'sell')
            ->where('transactions.status', 'final')
            ->select(
                'transactions.sub_type as name',
                DB::raw('COUNT(DISTINCT transactions.id) as count'),
                DB::raw('SUM(transactions.final_total) as amount') // Removed DISTINCT here
            );
        ...
        // Group by service_name (transactions.sub_type) and get the results
        $sell_details = $query->groupBy('transactions.sub_type')->get(); // Use get() instead of first()

        return $sell_details;
    }

    public function getSessionClinic($business_id, $start_date = null, $end_date = null, $location_id = null, $created_by = null, $permitted_locations = null)
    {
        $query = Transaction::where('transactions.business_id', $business_id)
            ->leftJoin('patient_session_info as psi', 'transactions.id', '=', 'psi.transaction_id')
            ->leftJoin('session_information', 'psi.session_id', '=', 'session_information.id')
            ->where('transactions.type', 'sell')
            ->where('transactions.status', 'final')
            ->where('transactions.sub_type', 'consultation')
            ->whereNotNull('session_information.session_name')
            ->select(
                'session_information.session_name as name',
                DB::raw('COUNT(DISTINCT transactions.id) as count'),
        ...
        if (!empty($permitted_locations) && is_array($permitted_locations) && $permitted_locations != 'all') {
            $query->whereIn('transactions.location_id', $permitted_locations);
        }
        ...
        // Group by session name and fetch results
        $sell_details = $query->groupBy('session_information.session_name')->get();

        return $sell_details;
    }

    public function getSellReturnClinic($business_id, $start_date = null, $end_date = null, $location_id = null, $created_by = null, $permitted_locations = null)
    {
        $query = Transaction::where('transactions.business_id', $business_id)
            ->leftJoin('transaction_payments AS TP', 'transactions.id', '=', 'TP.transaction_id')
            ->leftJoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
        ...
        $count_return_refund = $sell_details->count();
        $return_refund_amount = $sell_details->sum('amount');

        return [
            'sell_details' => $sell_details,
            'count_return_refund' => $count_return_refund ?? 0,
            'return_refund_amount' => $return_refund_amount ?? 0,
        ];
    }

    public function getPatientAppointments($business_id, $start_date = null, $end_date = null, $location_id = null, $doctor_id = null)
    {
        $query = DB::table('patient_appointment_requests')
            ->join('patient_profiles', 'patient_appointment_requests.patient_profile_id', '=', 'patient_profiles.id')
            ->join('transactions', 'patient_appointment_requests.bill_no', '=', 'transactions.id')
            ->where('transactions.business_id', $business_id)
            ->where('transactions.type', 'sell')
            ->where('transactions.status', 'final')
            ->where('transactions.sub_type', 'consultation')
            ->select(
                DB::raw("CASE 
                    WHEN (
                        ...
                        WHERE prev.patient_profile_id = patient_profiles.id 
                        AND prev.id < patient_appointment_requests.id
                    ) = 0 THEN 'New'
                    WHEN (
                        ...
                        WHERE prev.patient_profile_id = patient_profiles.id
                        AND prev.id < patient_appointment_requests.id
                    ) >= DATE_SUB(patient_appointment_requests.request_date, INTERVAL 4 MONTH) THEN 'Followup'
                    ELSE 'Old'
                END as patient_type"),
                DB::raw('COUNT(DISTINCT transactions.id) as count'),
                DB::raw('SUM(transactions.final_total) as amount')
            )
            ->groupBy('patient_type'); // Group by patient_type
    ...
    public function getTherapyClinic($business_id, $start_date = null, $end_date = null, $location_id = null, $created_by = null, $permitted_locations = null)
    {
        $query = Transaction::where('transactions.business_id', $business_id)
            ...
            ->select(
                'products.name',
                DB::raw('COUNT(DISTINCT transactions.id) as therapy_bill_count'),
                DB::raw('SUM(tsl.quantity) as therapy_item_count'),
                DB::raw('SUM(transactions.final_total) as amount')
            );
        ...
        $sell_details = $query->groupBy('products.name')->get();

        return $sell_details;
    }

    public function getTestClinic($business_id, $start_date = null, $end_date = null, $location_id = null, $created_by = null, $permitted_locations = null)
    {
        $query = Transaction::where('transactions.business_id', $business_id)
            ->leftJoin('transaction_sell_lines as tsl', 'transactions.id', '=', 'tsl.transaction_id')
            ->leftJoin('products', 'tsl.product_id', '=', 'products.id')
            ...
            ->select(
                ...
                DB::raw('COUNT(DISTINCT transactions.id) as test_bill_count'),
                DB::raw('SUM(tsl.quantity) as test_item_count'),
                DB::raw('SUM(tsl.quantity * tsl.unit_price_inc_tax) as amount')
            );
        ...
        $sell_details = $query->groupBy(DB::raw("COALESCE(categories.name, 'Uncategorized')"))
                            ->get();
        
        return $sell_details;
    }
    public function getTestBillClinic($business_id, $start_date = null, $end_date = null, $location_id = null, $created_by = null, $permitted_locations = null)
    {
        $query = Transaction::where('transactions.business_id', $business_id)
            ->where('transactions.type', 'sell')
            ->where('transactions.status', 'final')
        ...
        $sell_details = $query->groupBy('transactions.id')->get();

        return $sell_details;
    }
    public function getIpdClinic($business_id, $start_date = null, $end_date = null, $location_id = null, $created_by = null, $permitted_locations = null)
    {
        $query = Transaction::where('transactions.business_id', $business_id)
            ->leftJoin('transaction_sell_lines as tsl', 'transactions.id', '=', 'tsl.transaction_id')
            ->leftJoin('products', 'tsl.product_id', '=', 'products.id')
        ...
        // Group by service_name (products.name) and get the results
        $sell_details = $query->groupBy('products.name')->get(); // Use get() instead of first()
    ...
    public function deleteSale($business_id, $transaction_id)
    {
        ...
        $transaction = Transaction::where('id', $transaction_id)
            ...
            ->with(['sell_lines', 'payment_lines', 'cash_register_payments.cash_register'])
            ->first();
        ...
        if (!empty($transaction)) {
            ...
            if ($transaction->status == 'draft') {
                ...
                // Process refunds for cash register payments
                foreach ($transaction->cash_register_payments as $payment) {
                    if ($payment->cash_register) {
                        // Create a debit entry for the refund
                        $payment->cash_register->cash_register_transactions()->create([
                        ...
                        // Log an error or handle the case where cash_register is null
                        \Log::error('Cash register not found for payment ID: ' . $payment->id);
                    }
                }
    ...
    public function addSellReturn($input, $business_id, $user_id, $uf_number = true)
    {
        ...
        // Calculate rounded final total
        $rounded_final_total = round($invoice_total['final_total']);
        $round_off_amount = $rounded_final_total - $invoice_total['final_total'];
        ...
        $sell_return_data = [
            ...
            'tax_amount' => $invoice_total['tax'],
            'total_before_tax' => $invoice_total['total_before_tax'],
            'final_total' => $rounded_final_total,  // Rounded final total
            'round_off_amount' => $round_off_amount, // Round-off difference
        ];
        ...
        if (empty($sell_return)) {
            ...
            $sell_return_data['sub_type'] = $sell->sub_type;
    ...
    public function registerReport($business_id, $permitted_locations, $start_date = null, $end_date = null)
    {
        ...
        // Filter by user ID
        $user_id = request()->input('user_id'); // Ensure this matches your JS
        if (!empty($user_id)) {
            $registers->where('cash_registers.user_id', $user_id);
        }

        // Filter by status
        $status = request()->input('status'); // Ensure this matches your JS
        if (!empty($status)) {
            $registers->where('cash_registers.status', $status);
        }
        ...
        // Log final query
        \Log::info($registers->toSql(), $registers->getBindings());
        $results = $registers->get();
