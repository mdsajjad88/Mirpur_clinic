...
class Util
{
    ...
    public function calculate_end_datetime($start_datetime, $duration_text)
    {
        $carbon_start = Carbon::parse($start_datetime);

        // Match the duration (e.g. '5 minutes', '1 hour', '2 days')
        preg_match('/(\d+)\s+(minute|hour|day)s?/i', strtolower($duration_text), $matches);

        if (count($matches) == 3) {
            $value = (int) $matches[1];
            $unit = $matches[2];

            return $carbon_start->copy()->add($unit, $value)->format('Y-m-d H:i:s');
        }

        // fallback: return start time if invalid format
    ...
    public function is_doctor($user, $business_id = null)
    {
        $business_id = empty($business_id) ? $user->business_id : $business_id;

        return $user->hasRole('Doctor#' . $business_id) ? true : false;
    }
    ...
    public function sendSms($data)
    {
        ...
        // Log::info(['sms_settings'=>$sms_settings['send_to_param_name']]);
        // Log::info(['sms_settings'=>$sms_settings['msg_param_name']]);
        $options = [];
        if (! empty($headers)) {
        ...
        // Extract values for custom request data
        $user = isset($sms_settings['send_to_param_name']) ? $sms_settings['send_to_param_name'] : null;
        $password = isset($sms_settings['msg_param_name']) ? $sms_settings['msg_param_name'] : null;
        $from = isset($sms_settings['param_val_1']) ? $sms_settings['param_val_1'] : null;
        $to = isset($data['mobile_number']) ? $data['mobile_number'] : null;
        $text = isset($data['sms_body']) ? $data['sms_body'] : null;

        // Check if required values are available
        if (empty($user) || empty($password) || empty($to) || empty($text)) {
            if (empty($user)) {
            ...
            if (empty($from)) {
                return "Sender ID (from) is empty";
            }
            
            if (empty($to)) {
                return "Recipient number (to) is empty";
            }
            
            if (empty($text)) {
                return "Message text (text) is empty";
            }
        ...
        if ($sms_settings['request_method'] == 'get') {
            $response = $client->get($sms_settings['url'] . '?' . http_build_query($request_data_custom));
        } else {
    ...
    public function getTrashedUserRoleName($user_id)
    {
        // Fetch the user including soft-deleted ones
        $user = User::withTrashed()->find($user_id); // Use find() instead of findOrFail
    ...
    public function parseNotifications($notifications)
    {
        ...
        $missingContactsCount = Contact::whereNull('contact_id')->count();
        if ($missingContactsCount > 0) {
            $notifications_data[] = [
                'msg' => "There are {$missingContactsCount} customers added without contact ID.",
                'icon_class' => 'fas fa-user-times bg-red',
                'link' => action([\App\Http\Controllers\ContactController::class, 'index']), // update if needed
                'read_at' => null,
