$(document).ready(function () {
    ...
    $(
        '#tax_id, select#discount_for, #discount_type, #discount_amount, input#shipping_charges, \
    ...
    // Function to toggle expiry date field requirement based on status
    function toggleExpiryRequired() {
        var status = $('#purchase_status').val();

        $('#purchase_entry_table tbody').find('.exp_date').each(function() {
            if (status === 'received') {
                $(this).prop('required', true); // Set as required
                $(this).closest('.input-group').find('.input-group-addon').removeClass('hide'); // Show the field
            } else {
                $(this).prop('required', false); // Remove required
                $(this).closest('.input-group').find('.input-group-addon').addClass('hide'); // Hide the field
            }
        });
    }

    // Run on page load and on change of purchase status
    toggleExpiryRequired();  // Call function on page load

    $('#purchase_status').on('change', function() {
        toggleExpiryRequired();  // Call function when status changes
    });
    ...
    $(document).on('change', 'input[type="checkbox"][name^="auto_exp"]', function () {
        let rowCount = $(this).attr('id').match(/\d+/); // Extract row count from the checkbox ID
        let expMonth = $(this).closest('tr').find('.expMonth').val();
        ...
        if ($(this).is(':checked')) {
            ...
            // Add 1.5 years (18 months) to the current date
            let expiryDate = new Date(currentDate.setMonth(currentDate.getMonth() + expMonth));
            
            // Format the expiry date as 'DD-MM-YYYY'
            let day = ("0" + expiryDate.getDate()).slice(-2);      //   Get day, ensure two digits
            let month = ("0" + (expiryDate.getMonth() + 1)).slice   (-2); // Get month, ensure two digits
            let year = expiryDate.getFullYear();                   //   Get full year
        
        let formattedExpiryDate = `${day}-${month}-${year}`;
            
            // Set the expiry date input field to the calculated date
            $('input[name="purchases[' + rowCount + '][exp_date]"]').val(formattedExpiryDate);
        } else {
            // If unchecked, clear the expiry date field
            $('input[name="purchases[' + rowCount + '][exp_date]"]').val('');
        }
    });
    ...
    $(document).on('change', '.purchase_unit_cost_after_tax', function () {
        ...
        if (row.find('input.default_sell_price').closest('td').find('.bdt-display').length) {
            ...
            var rounded_bdt_value = Math.round(bdt_value / 10) * 10;
            row.find('.bdt-display').text('BDT: ' + rounded_bdt_value.toFixed(2));
            // row.find('.tax-display').text('BDT: ' + tax_value.toFixed(2));
        }
    });
...
function update_grand_total() {
    ...
    var discount_for = $('select#discount_for').val();  // Get the selected value from the dropdown
    var discount_type = $('select#discount_type').val();  // Flat or percentage
    var discount_amount = __read_number($('input#discount_amount'), true);  // The discount value input by user
    var discount = 0;
    var total_st_before_tax = 0;

    // Calculate discount based on the "discount_for" selection
    if (discount_for == 'subtotal_before_tax') {
        // Calculate discount based on net cost for each product row
        $('#purchase_entry_table tbody')
            .find('tr')
            .each(function () {
                total_st_before_tax += __read_number($(this).find('.row_subtotal_before_tax_hidden'), true);
            });
        discount = __calculate_amount(discount_type, discount_amount, total_st_before_tax);
    } else {
        // Default: Calculate discount based on total subtotal
        discount = __calculate_amount(discount_type, discount_amount, total_subtotal);
    }
