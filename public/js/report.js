$(document).ready(function() {
 ...
 register_report_table = $('#register_report_table').DataTable({
    ...
    columns: [
        ...
        { data: 'credit_sales', name: 'credit_sales', orderable: false, searchable: false },
    ...
    "footerCallback": function ( row, data, start, end, display ) {
        ...
        for (var r in data){
            ...
            credit_sales += $(data[r].credit_sales).data('orig-value') ? 
            parseFloat($(data[r].credit_sales).data('orig-value')) : 0;
        }
        ...
        $('.footer_total_credit_sale').html(__currency_trans_from_en(credit_sales));
    },
});

 // Register report
 clinic_register_report_table = $('#clinic_register_report_table').DataTable({
    processing: true,
    serverSide: true,
    scrollY: "75vh",
    scrollX: true,
    scrollCollapse: true,
    ajax: '/reports/clinic-register-report',
    columns: [
        { data: 'created_at', name: 'created_at' },
        { data: 'closed_at', name: 'closed_at' },
        { data: 'location_name', name: 'bl.name' },
    ...
    if ($('table#product_sell_report_table').length == 1) {
        $('#product_sr_date_filter').daterangepicker(
            ...
            function(start, end) {
                ...
                product_sell_grouped_report_single_inactive.ajax.reload();
    ...
    product_sell_grouped_report_single = $('table#product_sell_grouped_single_report_table').DataTable({
        ...
        ajax: {
            ...
            data: function(d) {
                ...
                var start_time = $('#product_sr_start_time').val();
                var end_time = $('#product_sr_end_time').val();
                if ($('#product_date_filter').val()) {
                    start = $('input#product_date_filter')
                        .data('daterangepicker')
                        .startDate.format('YYYY-MM-DD');
                    end = $('input#product_date_filter')
                        .data('daterangepicker')
                        .endDate.format('YYYY-MM-DD');
        ...
        columns: [
            { data: 'product_name', name: 'p.name', searchable: true, orderable: true },
            { data: 'sub_sku', name: 'v.sub_sku', searchable: true, orderable: true },
            { data: 'category_name', name: 'cat.name', searchable: false, orderable: true },
            { data: 'brand_name', name: 'b.name', searchable: false, orderable: true },
            { data: 'current_stock', name: 'current_stock', searchable: false, orderable: true },
            { data: 'total_qty_sold', name: 'total_qty_sold', searchable: false },
            { data: 'subtotal', name: 'subtotal', searchable: false },
        ],
        fnDrawCallback: function(oSettings) {
    ...
    product_sell_grouped_report_single_inactive = $('table#product_sell_inactive_report_table').DataTable({
        processing: true,
        serverSide: true,
        aaSorting: [[1, 'asc']],
        ajax: {
            url: '/reports/product-sell-grouped-report-inactive',
            data: function(d) {
                var start = '';
                var end = '';
                var start_time = $('#product_sr_start_time').val();
        ...
        columns: [
            ...
            { data: 'product_locations', name: 'product_locations', searchable: false, orderable: true },
            { data: 'sub_sku', name: 'v.sub_sku' },
            { data: 'category', name: 'category',searchable: false },
            { data: 'brand', name: 'brand', searchable: false, orderable: true },
            { data: 'current_stock', name: 'current_stock', searchable: false, orderable: true },
            { data: 'total_qty_sold', name: 'total_qty_sold', searchable: false },
            // { data: 'subtotal', name: 'subtotal', searchable: false },
        ],
...
function updateRegisterReport() {
    ...
    // Log the value of the select
    var user_ids = $('#register_user_id').val();
    console.log("User IDs: ", user_ids);

    var status = $('#register_status').val();

    var data = {
        user_id: user_ids, // Send array of IDs
        status: status,
        start_date: start,
        end_date: end
    };
...
function updateClinicRegisterReport() {
    ...
    var url_data = out.join('&');
    clinic_register_report_table.ajax.url('/reports/clinic-register-report?' + url_data).load();
}
