$(document).ready(function(){
	...
	*/
	...
	$(document).on('click', '.status_edit', function(e) {
		e.preventDefault();
		var url = $(this).data('href');
		$('.edit_schedule_status').load(url, function() {
			$(this).modal('show');
	...
	$(document).on('submit', 'form#edit_schedule_status', function(e) {
		e.preventDefault();
		var form = $(this);
		...
		$.ajax({
			...
			success: function(result) {
				if (result.success) {
					$('.edit_schedule_status').modal("hide");
					toastr.success(result.msg);
					
					// Reload the datatable or page as needed
					if (typeof follow_up_datatable !== 'undefined') {
						follow_up_datatable.ajax.reload();
					} else {
						// If no datatable variable exists, you might want to reload the page
						location.reload();
					}
				} else {
					toastr.error(result.msg);
	...
	*/
	...
	$('.schedule_log_modal').on('show.bs.modal', function (event) {
	    ...
	    $('form#schedule_log_form .select2').select2({
			dropdownParent: $('.schedule_log_modal')
		});
