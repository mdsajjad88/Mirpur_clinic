<div class="pos-tab-content">
    <div class="row">
        <div class="col-sm-4">
            <div class="form-group">
                <div class="checkbox">
                    <label>
                    {!! Form::checkbox('pos_settings[disable_direct_sell]', 1, !empty($pos_settings['disable_direct_sell']), [ 'class' => 'input-icheck', 'id' => 'disable_direct_sell']) !!} {{ __( 'lang_v1.disable_direct_sell' ) }}
                    </label>
                </div>
            </div>
        </div>        
    ...
    <div class="row">
        <div class="col-sm-4"> 
            <div class="form-group">
                {!! Form::label('auto_mfg', __('lang_v1.auto_mfg_location') . ':') !!} 
                @show_tooltip(__('lang_v1.auto_mfg_location_tooltip'))
                <div class="input-group">
                    <span class="input-group-addon">
                        <i class="fa fa-industry"></i>
                    </span>
                    {!! Form::select('pos_settings[auto_mfg]', $business_locations, $pos_settings['auto_mfg'] ?? '', ['class' => 'form-control select2', 'style' => 'width: 100%;', 'placeholder' => __('None')]) !!}
                </div>
            </div>        
        </div>
        <div class="col-sm-4"> 
            <div class="form-group">
                <div class="checkbox">
                    <label>
                        {!! Form::checkbox('pos_settings[not_for_sale]', 1, !empty($pos_settings['not_for_sale']), ['class' => 'input-icheck', 'id' => 'not_for_sale']) !!}
                        {{ __('Include only "Not for Sale" items in Manufacturing Recipes') }}
                    </label>
                </div>
            </div>
        </div>    
        <div class="col-sm-4">
            <div class="form-group">
                <div class="checkbox">
                    <label>
                        {!! Form::checkbox('pos_settings[allow_in_stock_to_demand_list]', 1, !empty($pos_settings['allow_in_stock_to_demand_list']), ['class' => 'input-icheck', 'id' => 'allow_in_stock_to_demand_list']) !!}
                        {{ __('Allow in-stock products to be added to demand list') }}
                    </label>
                </div>
            </div>
        </div>
        <div class="col-sm-4">
            <div class="form-group">
                <div class="checkbox">
                    <label>
                        {!! Form::checkbox('pos_settings[warn_on_walkin_customer]', 1, !empty($pos_settings['warn_on_walkin_customer']), ['class' => 'input-icheck', 'id' => 'warn_on_walkin_customer']) !!}
                        {{ __('Enable warning when billing to Walk-in Customer') }}
                    </label>
                </div>
            </div>
        </div>    
    ...
    <div class="row">
        ...
        <div class="col-sm-8">
            <div class="form-group">
                <table class="table table-bordered table-sm table-striped" id="discount_rules_table">
                    <thead>
                        <tr>
                            <th>Max Discount</th>
                            <th>Discount Type</th>
                        ...
                        @if (!empty($pos_settings['discount_rules']))
                            @foreach ($pos_settings['discount_rules'] as $index => $rule)
                                <tr>
                                    ...
                                    <td>
                                        {!! Form::select("pos_settings[discount_rules][$index][discount_type]", ['fixed' => 'Fixed', 'percentage' => 'Percentage'], $rule['discount_type'] ?? 'fixed', ['class' => 'form-control discount-type']) !!}
                                    </td>
                                    <td>
        ...
        <div class="col-sm-3">
            <div class="form-group">
                <div class="checkbox">
                    ...
                    @show_tooltip('This feature lets businesses set and apply discount rules automatically, ensuring consistent application of discounts as per defined policies. Only one discount type can be selected across all rows.')
                </div>
            </div>
        </div>
    </div>
