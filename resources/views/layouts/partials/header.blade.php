...
<header class="main-header no-print">
    ...
    <nav class="navbar navbar-static-top" role="navigation">
        ...
        @endif
        @if (env('APP_DEV') == true)
            <div class="logo" style="background-color: brown">Test Project {{ env('LAST_DEV') }}</div>
        @endif
        @php
            $is_doctor = false;
        ...
        <button style="margin-left: 20px; margin-top: 10px"
            class="btn @if ($is_doctor) @else hide @endif" id="doctor_status_indicator">
            <div class="toggle-wrapper">
                <label class="toggle-switch">
                    <input type="checkbox" id="doctor_status_toggle">
                    <span class="slider"></span>
                </label>
                <span id="doctor_status_text"></span>
            </div>
        </button>
        ...
        <div class="navbar-custom-menu">
            ...
            @php
                ...
                $register = App\CashRegister::where('user_id', $user_id)->where('status', 'open')->first();
            @endphp
            @if (($request->segment(1) == 'clinic-sell' || $request->segment(1) == 'pos') && session('register_open') == 1)
                ...
                @can('close_cash_register')
                    @if ($register)
                        <button type="button" id="close_register" title="{{ __('cash_register.close_register') }}"
                            ...
                            data-href="{{ action([\App\Http\Controllers\CashRegisterController::class, 'getClinicCloseRegister'], [$register->id]) }}">
                            <strong><i class="fa fa-window-close fa-lg"></i></strong>
                        </button>
                    @else
                        <button type="button" id="close_register" title="{{ __('cash_register.no_open_register') }}"
                            data-toggle="tooltip" data-placement="bottom"
                            class="btn btn-secondary btn-flat pull-left m-8 btn-sm mt-10" disabled>
                            <strong><i class="fa fa-window-close fa-lg"></i></strong>
                        </button>
                    @endif
                @endcan
