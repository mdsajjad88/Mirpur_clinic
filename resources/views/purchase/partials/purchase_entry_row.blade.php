        ...
        @if(session('business.enable_product_expiry'))
            <td style="text-align: left;">
                ...
                <div class="input-group">
                    ...
                    @if(!empty($common_settings['mandatory_expiry_date']) && $common_settings['mandatory_expiry_date'] == 1)
                        {!! Form::text('purchases[' . $row_count . '][exp_date]', $exp_date, [
                            'class' => 'form-control input-sm expiry_datepicker exp_date', 
                            'required',  // Makes the expiry date field mandatory
                            'readonly'
                        ]) !!}
                    @else
                        {!! Form::text('purchases[' . $row_count . '][exp_date]', $exp_date, [
                ...
                @foreach($common_settings['purchases_exp'] as $exp)
                @if (!empty($exp['category_id']) && $product->category_id == $exp['category_id'])
                <br>
                    <div class="">
                        <input hidden type="checkbox" class="input-sm auto_exp" name="auto_exp" id="auto_exp['{{ $row_count }}']">
                        <label for="auto_exp['{{ $row_count }}']" class="btn bg-info btn-xs">Set EXP Date</label>
                    </div>    
                    <input hidden type="number" name="expMonth['{{ $row_count }}']" id="expMonth['{{ $row_count }}']" class="expMonth" value="{{ $exp['month'] }}">
                @endif
            @endforeach
