...
<div class="row">
    <div class="pos-form-actions">
        <div class="col-md-12">
            <div class="pos-form-actions-demand">
                @if ($is_mobile)
                ...
                @endif
                <span id="othersButton">
                    @if (!Gate::check('disable_draft') || auth()->user()->can('superadmin') || auth()->user()->can('admin'))
                        <button type="button"
                            class="@if ($is_mobile) col-xs-6 @endif btn bg-info text-white btn-default btn-flat other-buttons @if ($pos_settings['disable_draft'] != 0) hide @endif"
                            id="pos-draft" @if (!empty($only_payment)) disabled @endif><i
                                class="fas fa-edit"></i> @lang('sale.draft')</button>
                    @endif
                    @if (!Gate::check('disable_quotation') || auth()->user()->can('superadmin') || auth()->user()->can('admin'))
                        <button type="button"
                            class="btn btn-default bg-yellow btn-flat other-buttons @if ($is_mobile) col-xs-6 @endif"
                            id="pos-quotation" @if (!empty($only_payment)) disabled @endif><i
                                class="fas fa-edit"></i> @lang('lang_v1.quotation')</button>
                    @endif
                    @if (!Gate::check('disable_suspend_sale') || auth()->user()->can('superadmin') || auth()->user()->can('admin'))
                        @if (empty($pos_settings['disable_suspend']))
                            <button type="button"
                                class="@if ($is_mobile) col-xs-6 @endif other-buttons btn bg-red btn-default btn-flat no-print pos-express-finalize"
                    ...
                    @if (!Gate::check('disable_credit_sale') || auth()->user()->can('superadmin') || auth()->user()->can('admin'))
                        @if (empty($pos_settings['disable_credit_sale_button']))
                            ...
                            <button type="button"
                                class="btn bg-purple btn-default btn-flat no-print pos-express-finalize other-buttons @if ($is_mobile) col-xs-6 @endif"
                    ...
                    @if (!Gate::check('disable_card') || auth()->user()->can('superadmin') || auth()->user()->can('admin'))
                        <button type="button"
                            class="btn bg-maroon btn-default btn-flat no-print other-buttons @if (!empty($pos_settings['disable_suspend']))  @endif pos-express-finalize @if (!array_key_exists('card', $payment_types)) hide @endif @if ($is_mobile) col-xs-6 @endif"
                    ...
                    @if (!Gate::check('disable_card') || auth()->user()->can('superadmin') || auth()->user()->can('admin'))
                        <button type="button"
                            class="no-print @if (!empty($pos_settings['disable_suspend']))  @endif pos-express-finalize other-buttons @if (!array_key_exists('custom_pay_1', $payment_types)) hide @endif @if ($is_mobile) col-xs-6 @endif"
                            data-pay_method="custom_pay_1" title="Express checkout using Bkash">
                    ...
                    @if (!Gate::check('disable_express_checkout') || auth()->user()->can('superadmin') || auth()->user()->can('admin'))
                        <button type="button"
                            class="btn btn-success @if (!$is_mobile)  @endif btn-flat no-print other-buttons @if ($pos_settings['disable_express_checkout'] != 0 || !array_key_exists('cash', $payment_types)) hide @endif pos-express-finalize @if ($is_mobile) col-xs-6 @endif"
                    ...
                    @if (!Gate::check('disable_pay_checkout') || auth()->user()->can('superadmin') || auth()->user()->can('admin'))
                        <button type="button"
                            class="btn bg-navy btn-default @if (!$is_mobile)  @endif btn-flat no-print other-buttons @if ($pos_settings['disable_pay_checkout'] != 0) hide @endif @if ($is_mobile) col-xs-6 @endif"
                ...
                @if (!isset($pos_settings['hide_recent_trans']) || $pos_settings['hide_recent_trans'] == 0)
                    <button type="button"
                        class="pull-right btn btn-primary btn-flat @if ($is_mobile) col-xs-6 @endif"
				...
				<div class="pull-right" id="demandButton" style="display: none; margin-right: 20px;">
					@if (!Gate::check('disable_draft') || auth()->user()->can('superadmin') || auth()->user()->can('admin'))
						<button type="button"
							class="@if ($is_mobile) col-xs-6 @endif btn bg-navy text-white btn-default btn-flat @if ($pos_settings['disable_draft'] != 0) hide @endif"
							id="pos-demand" @if (!empty($only_payment)) disabled @endif><i
								class="fas fa-thumbtack"></i> @lang('Demand')</button>
					@endif
				</div>
            </div>
        </div>
