			...
			<tr>
				@if(!Gate::check('disable_discount') || auth()->user()->can('superadmin') || auth()->user()->can('admin'))
					<td>
						<b>
							...
							@if($is_discount_enabled)
								(-):
								@can('disable_discount_from_sale_screen')
								...
								<span id="total_discount">0</span> <br>
				...
				<td>
					<span>
						...
						<span id="shipping_charges_amount">{{ isset($transaction->shipping_charges) ? @num_format($transaction->shipping_charges) : 0 }}</span>
				...
				@endif
				<td>
					<b id="notes1">@lang('lang_v1.notes'):</b>
					<i class="fas fa-edit cursor-pointer" id="notes1_icon" title="@lang('lang_v1.edit_note')" aria-hidden="true" data-toggle="modal" data-target="#notes1Modal"></i>
					<input type="hidden" name="notes1" id="notes1">
				</td>				
			</tr>
		</table>
	</div>
...
@include('sale_pos.partials.edit_notes1_modal')
