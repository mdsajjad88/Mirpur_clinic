...
<tr data-category="{{$product->category_name}}" class="product_row" data-row_index="{{$row_count}}" @if(!empty($so_line)) data-so_id="{{$so_line->transaction_id}}" @endif>
	...
	@if(!empty($is_direct_sell))
		...
		<td class="">
			<input type="text" name="products[{{$row_count}}][unit_price]" class="form-control pos_unit_price input_number mousetrap" value="{{@num_format($pos_unit_price)}}"@if(!auth()->user()->can('edit_product_price_from_sale_screen')) readonly @endif @if(!empty($pos_settings['enable_msp'])) data-rule-min-value="{{$pos_unit_price}}" data-msg-min-value="{{__('lang_v1.minimum_selling_price_error_msg', ['price' => @num_format($pos_unit_price)])}}" @endif> 
	...
	<td class="{{$hide_tax}}">
        ...
        <small style="display: block; margin-top: 10px;">
            <strong>Camping Discount ({{ $discount->name }}):</strong> {{ number_format($discount_amount) }}{{ $discount_type == 'percentage' ? '%' : 'TK' }} <br>
