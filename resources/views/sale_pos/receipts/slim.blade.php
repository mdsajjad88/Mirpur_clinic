    ...
    <div class="watermark-container">
            ...
            <thead class="border-bottom-dotted border-top">
                <tr>
                    ...
                    @if (empty($receipt_details->hide_price))
                        ...
                        @endif --}}
                        <th class="price text-right" width="14%">{{ $receipt_details->table_subtotal_label }}</th>
                    @endif
                </tr>
            </thead>
            <tbody>
                @forelse($receipt_details->lines as $line)
                    <tr class="border-bottom-dotted">
                        ...
                        <td class="description" style="font-size: 11px">
                            ...
                            @if (!empty($line['sub_sku']))
                                ...
                                @endif @if (!empty($line['brand']))
                                    ...
                                    @if ($line['unit_price_inc_tax_uf'] != $line['unit_price_exc_tax_uf'])
                                        <sup>(T)</sup>
                                    @endif
                                    @if ($line['unit_price_inc_tax_before_discount_uf'] != $line['unit_price_inc_tax_after_discount_uf'])
                                        <sup>(D)</sup>
                                    @endif
                        </td>
                        <td class="quantity text-center" >{{ number_format($line['quantity']) }} ‍
                            @if ($receipt_details->show_base_unit_details && $line['quantity'] && $line['base_unit_multiplier'] !== 1)
                                <br><small>
                        ...
                        @if (empty($receipt_details->hide_price))
                            <td class="unit_price text-center">
                                {{ number_format($line['unit_price_inc_tax_before_discount_uf']) }}
                            </td>
                    ...
                    @if (!empty($line['modifiers']))
                        @foreach ($line['modifiers'] as $modifier)
                            <tr class="border-bottom-dotted">
                                <td style="padding-left: 20px;"> <!-- Indentation for modifiers -->
                                    {{ $modifier['name'] }} {{ $modifier['variation'] }}
                                    @if (!empty($modifier['sub_sku'])) , {{ $modifier['sub_sku'] }} @endif
                                    @if (!empty($modifier['cat_code'])) , {{ $modifier['cat_code'] }} @endif
                                    @if (!empty($modifier['sell_line_note'])) ({{ $modifier['sell_line_note'] }}) @endif
                                </td>
                                <td class="text-center">{{ number_format($modifier['quantity']) }}</td>
                                @if (empty($receipt_details->hide_price))
                                    <td class="text-center">{{ number_format($modifier['unit_price_inc_tax']) }}</td>
                                    <td class="text-right">{{ number_format($modifier['line_total']) }}</td>
                                @endif
                            </tr>
                        @endforeach
                    @endif
        ...
        @if (empty($receipt_details->hide_price))
            ...
            <div class="flex-box">
                ...
                <p class="width-30 text-right ">
                    ৳ {{ number_format($receipt_details->subtotal?:0 + $receipt_details->total_line_discount?:0) }}
                </p>
            </div>
            ...
            @if (!empty($receipt_details->total_line_discount))
                <div class="flex-box">
                    ...
                    <p class="width-30 text-right">
                        (-) {{ number_format($receipt_details->total_line_discount) }}
                    </p>
                </div>
            @endif
            <!-- Discount -->
            ...
            @if ($receipt_details->round_off_amount != 0)
                <div class="flex-box">
                    ...
                    <p class="width-30 text-right font-family-1">
